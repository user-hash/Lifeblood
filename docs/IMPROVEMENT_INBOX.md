...
The 2026-05-19 two-phase hardening plan (`D:/Projekti/lifeblood_plan.txt`) lands its Phase-1 trust-hardening atoms (F0..F3f + S4..S8a), four native-clang adapter refactors (Phase 9), three execute-robustness fixes surfaced by Unity-IL2CPP DAWG dogfood (LB-LIM-004 a/b), and one post-release-wall dogfood find (F1d / `INV-EXTRACT-PROPERTY-READ-001`) that closed an 88.7%-workspace-wide property-edge gap. **Tests 1,011 → 1,097 (+86), zero skipped. Invariants 101 → 122 (+21) across 63 → 83 categories. Symbols 3,278 → 3,499 (+221). Edges 16,973 → 21,121 (+4,148, +24% — F1d closing the bare-identifier property/event-read gap on Lifeblood self). Types 350 → 363. Ports 26 → 27 (F3a adds `IPortHealthAnalyzer`).** Every Open LB-TRACK entry from the plan baseline flipped to Shipped after live-MCP roundtrip against DAWG verified the expected wire shape and behavior. The full per-atom catalogue lives in `CHANGELOG.md` `[Unreleased]`; the DAWG verification numbers live in `docs/DOGFOOD_FINDINGS.md` Session 11. The F1d find is the single most impactful: bare-identifier sibling-member property/event reads were emitting no graph edge, splitting Lifeblood's read-tool reality between `find_references` (which walks the semantic model directly) and `dependants` / `dead_code` / `blast_radius` / `port_health` (all edge-graph walkers); F2's `sameClassConsumerCount` triage field consequently always reported 0 on private property findings, defeating the FP-folding contract. Post-fix DAWG re-verification: edges 238,242 → 242,233 (+3,991 recovered property/event-read edges); workspace property zero-incoming rate 88.7% → 67.2% (-21.5 pp, 266 properties recovered); `dead_code` property finding count 247 → 62 (-75%). LB-TRACK-023 (canonical-id parity) remains Partially Shipped per entry body: F1c closed the empirical `.ctor`/`.cctor` parser bug; the deferred sub-atom is an SSoT consolidation parity-matrix ratchet — architectural hygiene with no known user-facing defect, acceptable to ship the trust-hardening slice without.
