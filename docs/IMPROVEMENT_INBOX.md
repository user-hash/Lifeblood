...
Active plan ledger: **`D:/Projekti/lifeblood_plan.txt`** (2026-05-18) superseded the earlier [`docs/plans/lifeblood-correctness-masterplan-2026-05-15.md`](plans/lifeblood-correctness-masterplan-2026-05-15.md). The 2026-05-19 close shipped the trust-hardening and adapter-thinning slice (`F0..F3f`, `S4..S8a`, native-clang adapter refactors, execute robustness, F1d, and the final canonical-id SSoT consolidation ratchet). Canonical per-entry truth lives in [`devmemory/lifeblood-tracking.md`](../devmemory/lifeblood-tracking.md). DAWG remains a dogfood/repro workspace; fixes from it must stay generic and ratchet-tested in Lifeblood.
...
## LB-TRACK-20260530-030. Compact diagnostic envelopes for repeated `compile_check` — **SHIPPED [Unreleased] (INV-DIAGNOSTIC-ENVELOPE-VERBOSITY-001)**
...
**Resolution.** Additive `verbosity:"compact"` on `compile_check` + `diagnose` (default verbose = byte-stable wire shape). Compact drops the full `definesActive[]` list and keeps `definesActiveCount`. Wire-shaping lives in `WriteToolHandler` (pure `ProjectDefines` seam), not the Domain DTO. `v1` input-schema snapshots updated; pinned by `ToolSchemaSnapshotTests`. Original entry below for regression-trace.
...
**Suggested fix shape.** Add an optional compact flag (for example `includeDefines:false`, `summarize:true`, or a stable `verbosity:"compact"` enum) to `lifeblood_compile_check` and `lifeblood_diagnose`. Preserve current verbose output by default for back-compat. Compact mode should retain `success`, diagnostics, `resolvedModule`, `existingTreeReplaced`, truth envelope, and a small defines summary (`count`, maybe `profileName`) while omitting the full `definesActive[]` list unless diagnostics require it.
...
## LB-TRACK-20260530-031. `lifeblood_execute` compiles against workspace/Unity types but cannot runtime-load them — opaque assembly-load error instead of a clear boundary — **SHIPPED [Unreleased] (INV-EXECUTE-WORKSPACE-LOAD-BOUNDARY-001)**

> Renumbered from `LB-TRACK-20260530-002` (2026-05-30 reconciliation) — see the note on `LB-TRACK-20260530-030`.

**Resolution.** Fix shapes #1 (document the boundary) + #2 (translate the loader error) shipped. `RoslynCodeExecutor` classifies the exception chain for a `FileLoadException`/`FileNotFoundException` whose assembly matches a known workspace module and returns a structured compile-against-not-run `targetRuntimeWarnings` boundary instead of the raw loader message; execute tool description + `DOGFOOD_CODE_EXECUTION.md` document it. Fix shape #3 (symbol-based `sizeof` helper) NOT shipped — out of cheap scope. Pinned by `ExecuteRobustnessTests`. Original entry below for regression-trace.
...
**Status (2026-05-24).** Strategic phase, not a single-fix entry. Open by design — the sequencing note on the original entry says "depends on everything else landing first". With L-LIM-001 (Wave 6 multi-define) now SHIPPED end-to-end but the Phase 3 versioned-schema sub-atoms still unshipped, the trust dashboard would still cite work-in-progress signals on the schema side. Substantial PR copy material accumulated (truth envelope, INV-LIST-SHAPE-UNIFORM-001 eternal ratchet, DAWG dogfood receipts in `devmemory/lifeblood-tracking.md`) is REGRESSION-TRACE substrate, not a public-facing narrative yet. Defer to post-v0.8.0-multi-define; the case-studies / comparison demos / trust dashboard work then has a defensible "what is this for / why trust it / where not yet" answer for every dimension. Treat this entry as a holding pen for the public-credibility work, not as actionable backlog.
...
## LB-INBOX-012. Full multi-profile analyze can throw NullReference after Unity asset import churn — **SHIPPED [Unreleased] (INV-ANALYZE-STRUCTURED-FAILURE-001)**

**Resolution.** Fix shapes #1 (wrap the full-analyze pipeline with a structured diagnostic envelope) + #3 (structured failure preserves context) shipped. `RoslynWorkspaceAnalyzer.AnalyzeWorkspace` tracks a coarse progress cursor (phase / module / file / profile / reached-compilation) and wraps any UNEXPECTED fault in a new language-agnostic `WorkspaceAnalysisException`; `ToolHandler.HandleAnalyze` serializes `{phase, module, filePath, profile, failedBeforeCompilation, exceptionType, message}` instead of the bare NRE. Deliberate `ArgumentException` validation propagates unchanged. Fix shape #2 (Unity-churn regression fixture) approximated by a faulting-resolver unit test — `WorkspaceAnalysisFailureTests` — rather than a full Unity-asset-churn harness. Original entry below for regression-trace.
