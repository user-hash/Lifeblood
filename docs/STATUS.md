...
| Lifeblood.Server.Mcp | MCP server with **25 tools** over stdio (12 read-side + 10 write-side). Bidirectional Roslyn. `RoslynSemanticView` constructed once per `GraphSession.Load` and shared by reference across consumers. McpDispatcher owns the wire protocol; Program.cs is a thin stdio I/O loop. |
