...
Dogfood-verified. 592 tests. **22 MCP tools** (12 read + 10 write). **23 port interfaces**. Native usage and timing reporting on every `lifeblood_analyze` response. Architectural-invariant introspection via `lifeblood_invariant_check` against 65 typed invariants in CLAUDE.md. CI green on Linux + Windows (4 jobs: build, TypeScript adapter, Python adapter, dogfood). Published on [NuGet](https://www.nuget.org/packages/Lifeblood).

<!-- portCount: 23 --><!-- testCount: 592 --><!-- toolCount: 22 -->
