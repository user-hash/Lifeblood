...
Dogfood-verified. 608 tests. **22 MCP tools** (12 read + 10 write). **24 port interfaces**. Native usage and timing reporting on every `lifeblood_analyze` response. Architectural-invariant introspection via `lifeblood_invariant_check` against 67 typed invariants in CLAUDE.md. CI green on Linux + Windows (4 jobs: build, TypeScript adapter, Python adapter, dogfood). Published on [NuGet](https://www.nuget.org/packages/Lifeblood).

<!-- portCount: 24 --><!-- testCount: 608 --><!-- toolCount: 22 -->
...
| Lifeblood.Application | **24 port interfaces** including `IWorkspaceAnalyzer`, `ICompilationHost`, `ISymbolResolver`, `ISemanticSearchProvider`, `IDeadCodeAnalyzer`, `IPartialViewBuilder`, `IResponseDecorator` (v0.6.7 truth envelope), `Invariants.IInvariantProvider`, `IUsageProbe` + `IUsageCapture`, `IFileSystem`, `IBlastRadiusProvider`, `IRuleProvider`, and the left-side adapter ports. Every read-side handler routes through `ISymbolResolver` before any graph or workspace lookup, and through `IResponseDecorator` after producing its raw result so every read-side response carries a truth envelope (truth tier, confidence, staleness, evidence source, limitations). Resolution order: canonical → truncated method → bare short name → kind correction (v0.6.6, `INV-RESOLVER-006`) → extracted short name from kind-prefixed / qualified input (v0.6.3, `INV-RESOLVER-005`). |
