...
Dogfood-verified. 582 tests. **22 MCP tools** (12 read + 10 write). **22 port interfaces**. Native usage and timing reporting on every `lifeblood_analyze` response. Architectural-invariant introspection via `lifeblood_invariant_check` against 64 typed invariants in CLAUDE.md. CI green on Linux + Windows (4 jobs: build, TypeScript adapter, Python adapter, dogfood). Published on [NuGet](https://www.nuget.org/packages/Lifeblood).

<!-- portCount: 22 --><!-- testCount: 582 --><!-- toolCount: 22 -->
