...
lock-name lists with one typed behavior registry. All 43 tools now declare
...
Wave 4 group 5 is verified by commit `dd55932` and the current alpha.90 build.
`routeFacts[]` evaluates required-per-route, consumer-selected parity, and
exact-target exclusive-owner policy through the existing call-route plan and
operation-fact stream. Synthetic engine/planner/validator fixtures plus a real
public MCP workspace exercised all three policies with no additional semantic
base. The full Release suite passed 1,681 tests with 11 explicit native-Clang
precondition skips (1,692 total). Live DAWG dogfood then proved fail-closed
input-drift rejection followed by an 83.1-second stable Editor publication and
a complete 102-module/4,370-file route-fact scan with
`additionalSemanticBaseCount:0`. Intake `012`, `023`, `025`, and `026` are
closed through consumer-authored composition; Lifeblood does not infer product
control laws, replay truth, sibling identity, or execution lanes.
...
cadence boundaries, multi-input/result/control operation shapes, exact branch
arms, forbidden-constant policy, request-local constant/source uniqueness, and
reasoned suppressions through one stateless
`ContractAuditEngine`. The engine derives a single selector-bounded fact query,
...
Disjunctive fact selectors keep mixed targeted calls, targetless element access,
and built-in shifts on one pass. Indexed pointers project to the same element
input shape while direct pointer dereference stays distinct; duplicate mask
positions are grouped only inside the bounded audit request. Enum usage and
static-table rows remain owned by their existing tools. Rule breakdowns derive
per-contract evaluated/finding-free/finding/suppressed counts from the same
evaluation counters, so zero matching occurrences cannot be mistaken for a
successful contract. Branch placement plus the existing value, source,
constant, loop, and threshold records cover lifecycle resets, declared
smoother routes, and advisory discontinuity shapes without another rule family
or retained fact base.
...
The same versioned contract surface now owns the comment-drift and invariant-
coverage projections. `sourceTextPolicies[]` returns caller-authorized advisory
comment/XML matches with exact source, nearby symbol, invariant context, and
suggested action. `invariantEvidence[]` joins the one parsed invariant
authority to lexical source/test references, bounded profile-applicable test
reachability, existing operation-contract breakdown rows, and explicitly
caller-declared external receipt references. Results are named evidence
categories plus concrete gaps and non-exclusive coverage states; Lifeblood
does not manufacture a global quality score or claim it executed external
receipts. One `ISourceEvidenceProvider` streams bounded facts from the retained
C# syntax trees with `additionalSemanticBaseCount:0`. Text/evidence-only
manifests mark the operation scan `NotRequested`; mixed manifests still use at
most one operation pass and one lexical pass over the same publication
(`INV-SOURCE-EVIDENCE-001`, `INV-CONTRACT-EVIDENCE-001`).
...
| Lifeblood.Adapters.Performance | Stateless generic JSON/CSV and structural Unity ProfilerRecorder import into the neutral request-local runtime-evidence model. It retains no captures and never interprets proprietary Unity profiler binaries. |
...
| Lifeblood.Tests | 1,709 discovered test cases: 1,698 pass and 11 native-clang environment gates skip when their executable precondition is absent. Coverage includes retained full/noop/edit/fallback memory receipts with start/end/delta/peak-above-start invariants, compact zero-heavy invariant-audit projection, first-class evidence-baseline drift parsing/policy/freshness/path/schema/Unity projection, bounded changed-set compile checks, diff-scoped diagnostic ownership and stale-diagnostic fail-closed behavior, stale-refresh target-diagnostic truth, retained and ephemeral secondary-profile contract audits, profile-aware bounded call routes with typed root evidence and direct/transitive membership, explicit targetless realtime operation policy, route-scoped shared-state risk classification, required/parity/owner-only route-fact policy, and the neutral operation-fact boundary and bounded contract-audit surface (including per-contract observation coverage, exact branch-arm placement, forbidden constants, lifecycle/smoothing/discontinuity shapes, near-equal literal grouping, exact cadence-boundary shapes, and buffer/index/mask contracts), recoverable Roslyn emit fallback, source-control root/failure/bounds receipts, latest-tag release metadata, atomic publication/read leases, snapshot preconditions, exact historical selection, bounded graph-only retention, forced-refresh read batches, canonical analysis identity/envelope projection, rule-only semantic sharing, exact cold/full and same-base incremental process coalescing, duplicate-preflight read prevention, persistent request cancellation, canonical accepted-change receipts, client leases, idle/maintenance drain, live shared status, waiter-aware cancellation/failure isolation, input-drift rejection, DevMemory authority, multi-profile applicability/scope safety, unsupported relationship receipts, Unity bridge schema/no-fixed-deadline/action-only-poll/conflict/shared-launch contracts, deterministic parallel extraction, registry routing, all-tools dispatch, process transport, architecture, documentation, packaging, and adapter contract ratchets. |
