...
| Lifeblood.Tests | 1497 tests (1486 passed + 11 native-clang `[SkippableFact]` environment gates). Coverage includes atomic publication/read leases, canonical analysis identity, rule-only semantic sharing, exact in-flight and process-level coalescing, persistent client leases, idle/maintenance drain, live shared status, waiter-aware cancellation/failure isolation, input-drift rejection, multi-profile scope safety, registry routing, all-tools dispatch, process transport, architecture, documentation, packaging, and adapter contract ratchets. |
...
totalCount    : 191
categories    : 138  (live category roster authoritative in `docs/invariants/INDEX.md`; sample no longer enumerated here so the body does not drift on every new invariant)
duplicates    : 0
parseWarnings : 0
sourcePaths   : [
