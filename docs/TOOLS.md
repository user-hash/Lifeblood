...
| **Contract audit** | Evaluate a schema-versioned consumer manifest over one target-filtered operation-fact stream. Families cover argument guards, consumer-annotated external API costs, and value domains with exact conversions, non-finite handling policy, and raw numeric-literal provenance. Nested constants distinguish literal, named, default, and folded origins plus finite/NaN/infinity classification; projects declare the acceptable policy/evidence and exact literal exceptions. Exact suppressions are versioned in the manifest. Summary-first by default, bounded evidence on request, and no additional retained semantic base. |
