...
Lifeblood exposes the **MCP tool surface** — read side + write side — in one shared session against a single loaded workspace. Live read / write counts in [`STATUS.md`](STATUS.md).
...
Every read-side tool response carries a top-level `envelope` field (`INV-ENVELOPE-001`) with `truthTier` (Semantic / Derived / Heuristic / Inferred), `confidence` (Proven / Advisory / Speculative), `evidenceSource` (Semantic / Inferred / Heuristic / RuntimeExternal), `stalenessSeconds`, `filesChangedSinceAnalyze`, per-tool `limitations[]`, and the exact `snapshotId`, `analysisGeneration`, and bounded `analysisIdentity` used by the response. Errors deliberately do NOT carry envelopes. Per-tool classification lives on `ToolDefinition.EnvelopeClassification` in the registry, projected into the decorator at startup, so registry and decorator cannot drift.

## Write-side (compiler-as-a-service)

| Tool | What it does |
|------|-------------|
| **Execute** | Run C# code against your actual project types, in-process. Access any class, call any method, inspect retained semantic state. `host` is the execution profile; optional non-host `targetProfile` values are accepted as compatibility hints, still run against the host scripting BCL, and surface the limitation on `targetRuntimeWarnings`. On Unity workspaces (`Library/` exists at the project root) DLLs from `Library/ScriptAssemblies/`, `Library/Bee/artifacts/`, and `Library/PackageCache/` are auto-injected; non-managed PEs, runtime BCL/contract assemblies, duplicate assembly identities, and retained workspace assemblies are filtered before Roslyn sees the probe set. Empty `Library/` surfaces a `runtimeAssemblyWarnings` entry. Sandbox globals: `Graph`, `Compilations`, `ModuleDependencies`, plus `Help` (cheat sheet), `SymbolsOfKind(string)`, `EdgesOfKind(string)`. CS1061 compile failures keep the raw Roslyn diagnostic and add a scripting-surface hint naming public members on the receiver type when available (`INV-EXECUTE-CS1061-HINT-001`). |
...
| **Static tables** | Extract `static` field / property collection-shaped initializers on a type as typed row + cell facts. Reads from `SemanticModel.GetOperation` only (no regex, no syntax-text parsing, no static-constructor execution). Each table reports container kind (`Array` / `CollectionExpression` / `ObjectCreation`), ordered rows in source order, and per-cell classification into `Null` / `Bool` / `String` / `Number` / `EnumMember` / `EnumFlags` (`Bits.A | Bits.B` flattened to a leaf id list in authoring order) / `MethodGroup` (delegate-target method id only — body content is dataflow, a separate truth tier) / `FieldReference` / `Array` (nested literal arrays recursively classified) / `Computed` (eternal fallback — caller reads `rawText` for source span). Constructor rows bind cells by parameter name + ordinal with `argumentKind` mirroring Roslyn's `Explicit` / `DefaultValue` / `ParamArray`; literal-array rows carry their value on `row.value` instead. Caps via `memberName` / `maxRows` (default tracks `RoslynStaticTableExtractor.DefaultMaxRows` — see [`STATUS.md`](STATUS.md)) / `maxTables` (default tracks `RoslynStaticTableExtractor.DefaultMaxTables`); zero / negative caps clamp to the default. The extractor is generic — no consumer-domain vocabulary in source, pinned by `StaticTableNameLeakageTests` (`INV-EXTRACT-STATIC-TABLES-001`). |
| **Assignment coverage** | Per-construction-site slot-coverage for a target type. For each `new TargetType { ... }` or `new TargetType()` + statement-level assignment site, walks the containing method's IOperation tree and reports which of the target's public mutable slot members (default: delegate-typed Func/Action/custom-delegate fields and properties — the Bindings shape) are assigned at that site and which are absent. Per-site `confidence` reflects construction shape: inline object-initializer OR non-aliased single-method statement-level assignment chain before escape is `Proven`; factory-constructed, aliased, or branched MAY-assign sites are `Advisory` with the bumping shape named in `siteLimitations[]` (`FactoryConstruction` / `AliasedLocal` / `BranchedMayAssign` / `PostEscapeAssignment`). Per-slot `status` is `Assigned` / `Absent` / `AssignedNull` (null-literal assignment is distinct from absent so a caller can tell 'forgot to wire' from 'deliberately wired null'). Per-slot `expressionKind` classifies the assignment expression (`Lambda` / `MethodGroup` / `FieldReference` / `PropertyAccess` / `NullLiteral` / `Other`). Operation-tree only — never regex, never syntax-text. `targetTypeId` accepts canonical (`type:NS.T`), fully-qualified, or short name — routed through the same resolver as `lifeblood_static_tables`. Optional flags toggle coverage on public mutable non-delegate fields / properties; `slotName` narrows to one slot; `maxSites` (default 256) clamps result size. `INV-ASSIGNMENT-COVERAGE-001..004`. |
| **Callsite arguments** | Per-call-site argument facts for a target method or constructor. Reports supplied vs default-filled arguments, parameter binding, classified value kind, clipped raw text, and per-parameter supplied/omitted histograms. Use it to verify whether a new optional parameter or overload is actually adopted by callers. |
| **Contract audit** | Evaluate a schema-versioned consumer manifest over one selector-bounded operation-fact stream. Families cover argument guards, consumer-annotated external API costs, call-route shared-state classification, required/parity/exclusive-owner route facts, value domains with exact conversion/non-finite/literal/cadence policy, and exact operation shapes. Optional `callRoutes[]` declare exact method roots plus depth/member bounds; Lifeblood derives deterministic shortest direct/transitive membership from the selected profile's existing semantic `Calls` edges. `externalApiCosts[].callRouteIds` then narrows occurrences to that execution scope and findings carry typed root, distance, placement, and path evidence. External-cost contracts select exactly one target policy: explicit `targetSymbolIds[]`, or `matchAnyTarget:true` for targetless/all-target operation kinds such as array/object/delegate creation, interpolation, throw, await, and lock. Empty targets never imply a wildcard. Targeted entries cover consumer-annotated LINQ, logging, reflection, Unity, and other forbidden APIs through the same rule. This is a request-local query plan, never another graph, call-edge authority, cache, or retained semantic base. An `operationShapes[]` contract selects operation kinds plus optional target, containing-symbol, and operator ids; each `allowedShapes[]` alternative may constrain several input roles/ordinals, allowed value kinds/types/constants, required/forbidden constants, required source symbols/operators, result types, lexical control contexts, and exact `Condition` / `WhenTrue` / `WhenFalse` branch placement. This lets consumer manifests express lifecycle resets, required smoother routes in selected loops, and advisory hard-gate/discontinuity policy without a temporal analyzer or product-name inference. Exact threshold shapes reuse value-domain boundary policy. An optional `uniquenessPolicy` groups a selected input by `ConstantValue` or `SourceSymbolId` only for the bounded request, enabling duplicate mask-position detection without a retained fact cache. Each rule row includes per-contract evaluated, occurrence-local finding-free, finding, and suppressed counts, making a zero-match evidence gap explicit. Array, span/native-container calls, targetless element access, indexed pointers, and integer shifts use the same neutral shape; direct `*pointer` remains `PointerIndirection`. Existing reference/dependant, enum-coverage, and static-table tools remain their fact authorities rather than being copied into this rule. Nested constants distinguish literal, named, default, and folded origins plus finite/NaN/infinity classification; comparison facts retain exact left/right value provenance. Exact suppressions are versioned in the manifest. `profileScope` may select any profile in the committed analysis: the retained profile reuses its compilation set; a secondary profile verifies snapshot inputs, compiles the requested module scope sequentially, and releases it immediately. `sourceTextPolicies[]` adds caller-owned advisory retired-phrase matching over comments/XML docs with exact source and nearby-symbol evidence. `invariantEvidence[]` joins the one parsed invariant authority to lexical references, graph-addressable source/test symbols, bounded reachable tests, existing operation-contract breakdowns, and caller-declared external receipts. It reports one `invariantEvidencePolicies[]` selector receipt even when a prefix selects zero declarations, then named category statuses, concrete gaps, and coverage states without a global score. A text/evidence-only manifest reports the operation scan as `NotRequested`; its one bounded retained-tree scan reports `additionalSemanticBaseCount:0`. Summary-first by default, bounded evidence on request, and no additional retained semantic base. |
| **Wire audit** | Dead-wire audit: referenced members that are structurally unplugged. Flags read-without-write fields, never-assigned delegate slots, events subscribed but never raised, events raised but never subscribed, and private/internal methods whose call sites pass only degenerate constant/default/null arguments. Advisory for reflection, Unity serialized YAML, and runtime-procedural assignment. |
| **Feature-switch audit** | Finds boolean fields / settable boolean properties that gate branches but are pinned to their default because nothing reachable flips them. Verdicts are `AlwaysDefaultInGraph`, `TestOnlyActivation`, or `RuntimeMutable`; advisory for reflection, config/save-state, Unity serialized YAML, and out-of-graph callers. |
| **Member count** | Declared-member count for a type. `reflectionDeclared` reproduces `System.Reflection` DeclaredOnly counting bit-exactly; `sourceSymbols` returns Lifeblood graph child-symbol semantics. Useful for offline architecture-debt ratchets that cannot load the workspace assembly. |
| **Struct layout** | Computes struct metadata layout offline: layout kind, effective pack, declared size, total size, alignment, pointer size, unmanaged/blittable flags, and per-field offset / size / alignment / fixed-buffer length. Exact for known blittable Sequential / Explicit structs; Advisory with named limitations for Auto/reference/non-blittable/unknown shapes. |
...
### Contract Audit Route Facts

Use manifest `routeFacts[]` when several declared `callRoutes[]` must carry the
same semantic handoff, must expose equivalent selected operation shapes, or are
the only routes allowed to touch an exact target:

- `RequiredOnEveryRoute` reports each selected route with no matching fact.
- `EquivalentAcrossRoutes` compares multisets of consumer-selected signature
  dimensions such as target, input kinds/types/constants/sources/operators, and
  lexical control context. It does not claim broader behavioral equivalence.
- `AllowedRoutesOnly` reports matching calls or writes outside the declared
  owner routes. It requires exact target ids so the global bypass check remains
  bounded and reviewable.

Each receipt exposes selected fact/signature totals and per-route satisfaction.
Missing/parity/owner conclusions become Advisory when the fact scan or a route
is truncated. Put related external-cost, state-access, operation-shape, and
route-fact contracts in one manifest so they share the route plan, fact scan,
and any ephemeral secondary-profile compilation. This family does not infer
thread/audio/job lanes; the manifest names the meaningful routes and signature
dimensions.
...
| **Dead code** ¹ | **[EXPERIMENTAL. ADVISORY ONLY]** Scan the graph for symbols with no incoming semantic references. Consults `IUnityReachabilityProvider` (when injected) so MonoBehaviour magic methods, Unity entrypoint attributes, and resolved UnityEvent persistent-call targets are excluded as runtime-reachable. Every finding carries triage fields — `directDependants` (incoming non-Contains edge count, forward-compatible signal for future relaxed-criteria modes), `bucket` (`Production` / `Test` / `Editor` / `Generated` / `Vendored` segment-aware path classification, plus dead_code-only `Scaffolding` for intentional conditional/const-string anchor types), `declarationOnly` (true iff abstract — deleting one is a public-contract change). Response carries `bucketBreakdown` alongside `kindBreakdown` so a caller can fold the Editor/Generated/Vendored/Scaffolding tail without re-walking `findings[]` (`INV-DEADCODE-TRIAGE-001`, `INV-DEADCODE-SCAFFOLDING-001`). See the caveat below before acting on findings. |
| **Partial view** | Combined live source of every partial declaration of a type. Walks file-level `Contains` edges, reads each file via `IFileSystem`, emits per-segment source plus a concatenated combined view with file headers. Requires the latest workspace root and is deliberately unavailable on graph-only historical selection. |
| **Invariant check** | Query the architectural invariants declared anywhere in the loaded project's invariant tree. **Dynamic discovery (`LB-FR-023`)**: walks `<root>/CLAUDE.md`, `<root>/AGENTS.md`, and any `<root>/docs/invariants/**.md` tree via `IFileSystem`. Each source is parsed and aggregated; per-id duplicate detection spans the full set — within-file AND cross-file (e.g. a CLAUDE.md ↔ AGENTS.md mirror) — and each duplicate carries a per-site `occurrences[]` ledger (`sourcePath` + `line` + `title`) so a benign mirror is distinguishable from a real collision. `audit.SourcePaths[]` + `audit.SourceCounts[]` report contributing files and per-source declaration counts, and `declaredCount` (= Î£ `sourceCounts`) − `totalCount` (unique ids) = `duplicateDeclarationCount`, reconciling the per-source sums against the unique total. Audit responses also include `coverage` + `coverageWarnings` so prose-only or under-recognized source trees are visible without misusing parse warnings, plus a docs-safe `evidenceReceipt` with durable counts and a `doNotCite[]` list for volatile envelope metadata. Pass `summarize:true` in audit mode for the compact projection: totals, categories, duplicates with occurrences, warnings, and coverage stay top-level; only nonzero source counts appear once at `evidenceReceipt.sourceProjection`, with discovered/returned/zero/omitted counts and `truncated:false`; `sourceProjectionReference` names that single owner. Default audit remains the full v1 shape. **Five authoring shapes (`LB-BUG-017` / `LB-BUG-018`)**: A (`- **INV-X-N**: body`), B (`- **INV-X-N. Title.** Body`), C (`**INV-X-N: Title.** body` - bare bold paragraph, no bullet), D (`- **INV-X-N** (vX.Y.Z): body` - parenthesized version tag), E (`- **INV-X-N:** body` - colon inside the bold). Three modes: pass `id` to fetch one invariant's full body + title + category + source line; pass `mode="audit"` (default) for a summary with unique `totalCount`, `declaredCount`/`duplicateDeclarationCount`, coverage, per-category breakdown, duplicate-id collisions (with `occurrences[]` provenance), and parse warnings; pass `mode="list"` for every id + title index. Lifeblood-self audit counts are pinned in [`STATUS.md`](STATUS.md); per-workspace counts vary and are reported live. `INV-INVARIANT-001`, `INV-INVARIANT-002`. |
...
| **Authority coverage** | Check whether subjects reach intended authority symbols. Expands subject types/files to contained methods, expands authority types/files/namespaces to contained symbols, walks outgoing non-Contains edges, and reports reached vs missing required authorities plus shortest path previews and first allowed alternatives encountered. Use it for missing-dependency/source-of-truth audits, not as a verdict engine. `INV-AUTHORITY-COVERAGE-001`. |
...
- **Intentional scaffolding downrank** — `INV-DEADCODE-SCAFFOLDING-001` (`LB-INTAKE-20260608-003`). Non-public static types whose direct members are exclusively `[Conditional]` methods and/or static const string anchors, plus those direct members, are still reported but bucketed as `Scaffolding` instead of ordinary Production deletion candidates.
- **Resolved UnityEvent persistent calls** — `INV-UNITYEVENT-REACHABILITY-001` (`LB-INTAKE-20260601-001`). The Unity reachability adapter scans `.prefab` / `.unity` / `.asset` YAML, resolves `m_Script` GUIDs through `.cs.meta`, and marks resolved UnityEvent target methods plus host types reachable.
...
- Unresolved Unity serialized callbacks/asset references, serialized enum production, and reflection-based dispatch invisible to static analysis (`Type.GetType` + `MethodInfo.Invoke`, Unity `SendMessage`-dispatched handlers).
...
After the first `lifeblood_analyze`, subsequent calls with `incremental: true` only recompile modules whose source content changed since the last analysis. Filesystem mtimes are the cheap candidate prefilter; source content hashes decide whether a touched file actually re-extracts. Responses report `mtimeTouchedSourceFiles`, `contentChangedSourceFiles`, and `changedSourceFiles` so save-without-content-change is visible as an `incremental-noop`. Csproj timestamp changes (`INV-BCL-005`) and asmdef changes (`INV-UNITY-002`) still trigger per-module re-discovery so compilation facts do not go stale.

```
lifeblood_analyze projectPath="/my/project"                    -> full analysis (~14-34 s depending on workspace size)
lifeblood_analyze projectPath="/my/project" incremental=true   -> seconds when nothing changed; dirty source modules re-analyze by content hash
```
...
`analysisScopeChanged` / `compilationStateUnavailable` - asmdef edits, csproj structural changes, analyze
`excludePaths` scope changes, read-only-to-retained recovery, etc.) and the caller decides
what to do.
...
## Snapshot-pinned reads, history, and batches
...
Pass `snapshotId` to select one exact current or retained graph-only
publication. Missing or evicted ids return `failure:"snapshot-not-found"` with
the current id and retained inventory; a nested batch call cannot switch away
from its outer lease. Historical graph queries retain their normal response
shape and envelope. Historical publications deliberately contain neither
Roslyn services nor source bytes, so compilation-backed, `partial_view`, and
