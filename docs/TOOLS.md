...
| **Contract audit** | Evaluate a schema-versioned consumer manifest over one target-filtered operation-fact stream. Families cover argument guards, consumer-annotated external API costs, and value domains with exact conversions, non-finite handling policy, raw numeric-literal provenance, consumer-toleranced near-equal groups, and exact cadence-boundary shapes. Nested constants distinguish literal, named, default, and folded origins plus finite/NaN/infinity classification; comparison facts retain exact left/right value provenance. Projects declare acceptable policy/evidence, literal exceptions, and boundary forms. Exact suppressions are versioned in the manifest. `profileScope` may select any profile in the committed analysis: the retained profile reuses its compilation set; a secondary profile verifies snapshot inputs, compiles the requested module scope sequentially, and releases it immediately. `scanReceipt` reports available profiles, execution mode, input verification, compiled/scanned counts, and `additionalSemanticBaseCount:0`. Summary-first by default, bounded evidence on request, and no additional retained semantic base. |
