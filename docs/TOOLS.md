...
| Tool | What it does |
|------|-------------|
| **Capabilities** | Report the live MCP server's version, version source, optional git commit / dirty state when running from a repo checkout, tool count with read/write split, feature flags, schema snapshot path, STATUS.md anchor path, and current session state. Use at session start to detect local-server / local-doc drift before relying on stale prose. |
