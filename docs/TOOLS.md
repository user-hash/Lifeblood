...
| **Contract audit** | Evaluate a schema-versioned consumer manifest over one target-filtered operation-fact stream. Initial families cover argument guards and consumer-annotated external API costs; exact suppressions are versioned in the manifest. Summary-first by default, bounded evidence on request, and no additional retained semantic base. |
