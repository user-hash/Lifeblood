...
**Current state (post-v0.7.7):** 3,278 symbols, 16,973 edges, 11 modules, 350 types, 0 violations, 0 cycles. Verified on a real 90-module Unity workspace (63,274 symbols, 225,167 edges, 125 SCCs). **29 MCP tools (17 read + 12 write).** **1011 tests, zero skipped.** **26 port interfaces.** **101 typed invariants across 63 categories** under `docs/invariants/` tree (queryable via `lifeblood_invariant_check`, zero duplicates, zero parse warnings on Lifeblood self). **First non-C# adapter ships in v0.7.7: native-clang for C** (beta, libclang-backed external JSON producer at `adapters/native-clang/`; see `docs/NATIVE_CLANG.md`). +6 invariants since v0.7.3: `INV-DEADCODE-TRIAGE-001` (per-finding `directDependants` / `bucket` / `declarationOnly` triage fields), `INV-DIAGNOSTIC-ENVELOPE-DEFINES-001` (preprocessor scope on `diagnose` / `compile_check` responses), `INV-CYCLE-TAXONOMY-001` (`GeneratedOrStaticAnalysisArtifact` / `PartialClassCluster` / `LikelyRealLoop` classification on `cycles`), `INV-ENUM-COVERAGE-001` (new `lifeblood_enum_coverage` tool — per-member produced / consumed-comparison / consumed-switch classification), `INV-TEST-IMPACT-001` (new `lifeblood_test_impact` tool — which test classes transitively depend on a target), `INV-PATHBUCKET-SHARED-001` (Domain-layer SSoT for Production / Test / Editor / Generated path bucketing, replacing three drifted classifiers). CallSite provenance verified on a real Unity workspace — remainder are graph-derived edges with no authoring location, as designed.
