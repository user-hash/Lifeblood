...
**Current state (post-v0.7.7 + 2026-05-19 plan close, [Unreleased]):** 3,499 symbols, 21,121 edges, 11 modules, 363 types, 0 violations, 0 cycles. Verified on a real 90-module Unity workspace (DAWG, 65,940 symbols, 242,233 edges, 91 SCCs). **29 MCP tools (17 read + 12 write).** **1,097 tests, zero skipped.** **27 port interfaces** (port count grew with the `IPortHealthAnalyzer` seam landed by F3a). **122 typed invariants across 83 categories** under `docs/invariants/` tree (queryable via `lifeblood_invariant_check`, zero duplicates, zero parse warnings on Lifeblood self). **Native-clang adapter (beta)** at `adapters/native-clang/` continues to mature behind the `JsonGraphImporter` boundary; four 2026-05-19 refactors lift type-member / global / function emission and overall adapter boundary envelopes behind explicit facts. +13 invariants since v0.7.7 from the 2026-05-19 plan: `INV-EXTRACT-EXTENSION-REDUCED-001` (F1a), `INV-EXTRACT-CROSS-PARTIAL-RESOLUTION-001` (F1b), `INV-CANONICAL-ID-PARITY-001` (F1c), `INV-DEADCODE-TRIAGE-002` (F2), `INV-PORT-HEALTH-ANALYZER-SEAM-001` (F3a), `INV-PORT-HEALTH-COMPOSITE-001` (F3b), `INV-EXTRACT-IFACE-INHERIT-001` (F3c), `INV-AUTHORITY-COMPOSITE-001` (F3e), `INV-ENUM-COVERAGE-DISPATCH-TABLE-001` (S4), `INV-DIAGNOSE-FRESHNESS-001` (S5), `INV-DIAGNOSE-FRESHNESS-002` (S5b), `INV-ADVISORY-LIMITATIONS-001` (S6), `INV-AUTHORITY-PLANNING-COMPOSITION-001` (S7), `INV-ADAPTER-THIN-001` (S8a), `INV-ADAPTER-LOOKUP-SEAM-001` (S8a-fix), `INV-EXTRACT-PROPERTY-READ-001` (F1d — dogfood-derived). The F1d find is the most impactful: bare-identifier sibling-member property/event reads were dropping 88.7% of private-property incoming edges workspace-wide, splitting the read-tool reality between `find_references` (which walks the semantic model directly) and the edge-graph walkers (`dependants` / `dead_code` / `blast_radius` / `port_health`). Closed in the same release wall.

**Snapshot at v0.7.2 (2026-05-08, post G1+G2+G4+R2-3 wave + reviewer-catch UTF-16/incremental fixes):** 2,383 symbols, 11,720 edges, 11 modules, 272 types, 0 violations, 0 cycles. Verified on a real 90-module Unity workspace (DAWG, 60,775 symbols, 214,097 edges, 122 SCCs — edge count grew +18% over the prior baseline because enum-member references the dangling-edge filter was silently dropping `R2-3` now resolve). **25 MCP tools (15 read + 10 write).** All Roslyn capabilities at Proven. **751 tests.** **26 port interfaces.** **76 typed invariants under `docs/invariants/` tree** (8 domain files + INDEX), aggregated by the dynamic tree-walker that also picks up `<root>/CLAUDE.md` + `<root>/AGENTS.md` if they declare any (queryable via `lifeblood_invariant_check`, zero duplicates, zero parse warnings on Lifeblood self; 83 invariants discovered on DAWG across CLAUDE.md hot rules + AGENTS.md project instructions + `docs/invariants/**.md`). Truth envelope on every read-side response (`INV-ENVELOPE-001`). Caller-owned scope policy on incremental analyze (`INV-ANALYZE-FALLBACK-001`): rejects on detected drift unless `allowFullFallback` opted in; rejection wire shape carries `mode` + `requestedMode` + `fallbackReason` + `canRetryFull` + `suggestedRetry`. Enum members are first-class graph symbols (`INV-EXTRACT-ENUMMEMBER-001`); resolver Rule 4 short-name fallback refuses cross-type / cross-kind silent substitution for member-kind inputs (`INV-RESOLVER-007`). Search results carry `MatchKind` (`Name` / `QualifiedName` / `XmlDoc` / `Multiple`) so callers can filter by signal source without parsing snippet strings (`INV-SEARCH-MATCHKIND-001`). Unity-aware reachability (`INV-UNITY-001`) cuts MonoBehaviour magic-method false positives by 97% on real Unity workspaces, plus the `LB-FP-003` Editor reflection roster + type-via-child propagation closes the `[SettingsProvider]`-on-host-type FP class. Authority report + forwarder classifier (`INV-AUTHORITY-001`, `INV-FORWARDER-001`) classify methods across the full surface and identify forwarder candidates for host-type / dispatcher / partial-class extraction triage (the DAWG dogfood case ran an ABG partial-class wave, but the same metric drives any host-with-many-subordinates split decision — filter family selectors, kernel routers, capability dispatchers, facade types). `cycles` and `context` smart-dynamic shaping (`LB-FR-021`, `LB-FR-022`) keeps responses inside conservative tool-result budgets even on multi-module Unity workspaces. File-mode `compile_check` (`LB-BUG-019`) resolves the owning compilation and swaps the existing tree, so module-owned files compile-check against their real reference set.

### Session 11. 2026-05-19 two-phase plan close + F1d dogfood find

Phase 1 of the 2026-05-19 plan (`D:/Projekti/lifeblood_plan.txt`) shipped 14 atoms across canonical-symbol-identity hardening (F1a/b/c), dead-code triage (F2), composite-port intelligence (F3a..f), enum-coverage dispatch-table awareness (S4), diagnose freshness (S5/S5b), advisory-limitations envelope (S6), planning-verdict evidence (S7), and adapter thinning (S8a + S8a-fix + find_references service extraction). Phase 9 added four native-clang adapter refactors lifting type-member / global / function emission and overall boundary envelopes behind explicit facts. Three execute-robustness fixes (native-PE managed gate, script-reference-set isolation, target-profile honesty) closed the Unity-IL2CPP `lifeblood_execute` blocker surfaced during DAWG dogfood (LB-LIM-004 a/b). Then a post-release-wall DAWG re-verification surfaced F1d, the highest-impact extractor gap yet measured.

**F1d — bare-identifier property/event reads emit no graph edge (LB-TRACK-20260519-024).**

Empirical class. Live MCP roundtrip on DAWG (65,940 symbols / 90 modules) called `lifeblood_dead_code(includeKinds:[Property], excludePublic:true)` to verify F2's `sameClassConsumerCount` triage field. The field's wire-shape was present on every finding — but the value was 0 on every empirical case the tracker had cited. `lifeblood_find_references(property:Nebulae.BeatGrid.Audio.FX.BeatGridFXBusManager.DspTime)` returned 4 hits (declaration + 3 sibling reads with proper `containingSymbolId`); `lifeblood_dependants(...)` on the same symbol returned `count: 0`. The two read tools were giving contradictory answers about the same property.

Workspace-wide audit via `lifeblood_execute`:
| Symbol kind (non-public) | Zero incoming non-Contains edges |
|---|---:|
| Property | **88.7%** (1099 / 1239) |
| Field | 1.5% (174 / 11,765) |
| Type | 0.8% (4 / 482) |
| Method | 25.3% (2184 / 8649; legitimately mixed) |

The Property/Field delta isolated the bug. `RoslynEdgeExtractor.ExtractReferenceEdge` (the `IdentifierNameSyntax` walker) carried arms for `INamedTypeSymbol` / `IFieldSymbol` / `IMethodSymbol` but no `IPropertySymbol` / `IEventSymbol` arm. The member-access form (`this.X`, `obj.X`) was already handled by `ExtractMemberAccessEdge` through the shared `EmitSymbolLevelEdge` helper, so the wire-shape contract was intact but the AST-entry point was incomplete. C# style convention overwhelmingly drops the `this.` prefix, so the bare-identifier path is the common case and the gap silently swallowed ~89% of private-property incoming edges across a real workspace.

Roslyn's `SymbolFinder.FindReferencesAsync` walks the semantic model directly and ignores the graph, so `find_references` kept working while `dependants` / `dead_code` / `blast_radius` / `port_health` (all edge-graph walkers) systematically missed every bare-identifier property read. F2's `sameClassConsumerCount` triage field consequently always reported 0 on private property findings, defeating the FP-folding contract the field was added to deliver.

**Fix.** `RoslynEdgeExtractor.ExtractReferenceEdge` gains an `IPropertySymbol or IEventSymbol` arm that routes through `EmitSymbolLevelEdge` — same helper member-access uses, so the wire shape is byte-stable across both AST entry points. Covers reads AND writes (LHS of `=` is also `IdentifierNameSyntax`) AND event subscription (`Changed += h;`). Pinned by three `RoslynExtractorTests` fixtures (read / write / event-wire). `INV-EXTRACT-PROPERTY-READ-001`.

**Post-fix re-verification on DAWG (live MCP).**
- Edges: 238,242 → **242,233 (+3,991)** — recovered bare-identifier property/event-read edges.
- `dependants(...DspTime)`: 0 → **3** with full callSite metadata at lines 250/634/691.
- `dependants(...OverridesPath)`: 0 → **2** (per-source-method dedup correctly collapsing 3 grep reads in `SaveOverrides` to one entry + 1 read in `LoadOverrides`).
- Workspace property zero-incoming rate: 88.7% → **67.2%** (-21.5 pp absolute, 266 properties recovered).
- `dead_code` property finding count: 247 → **62** (-75%). DspTime + OverridesPath dropped from the FP set entirely. Remaining 62 are Unity SerializeField-backed Inspector-bound properties (different runtime-discovery class, not F1d scope).

**Companion atoms verified in the same DAWG roundtrip.** Every Open LB-TRACK entry from the 2026-05-19 plan baseline flipped to Shipped after live-MCP verification confirmed the expected wire shape and behavior on DAWG:

- LB-TRACK-012 (cross-partial private-method invocations) → F1b `b0b5eb5`: `EnableAutoRotationDelayed` returns 2 references — declaration at `AdaptiveBeatGrid.cs:282` + Usage at `AdaptiveBeatGrid.Bootstrap.Events.cs:109`.
- LB-TRACK-013 (port_health composite verdict) → F3a `fe50998` + F3b `8df96c6` + F3c `bf23480`: `ITransportTimelineHost` returns composite-aware healthy verdict with 24 aggregated members across 5 inherited sub-ports.
- LB-TRACK-014 (enum_coverage dispatch-table routing) → S4 `c3ee6dd`: `FeatureId` 33 members all carry `dispatchTableReferenceCount`; the 8 previously-mystery values now show ≥1.
- LB-TRACK-015 (dead_code same-class FPs) → F2 `8134337` wire-shape + F1d `7a45ff0` underlying-graph fix.
- LB-TRACK-017 (authority_report composite surface) → F3e `bd4c59f`: `AdaptiveBeatGrid` returns 7 / 142 / 1202 / 426 / 0.354 (EXACT match to Polish-1 P0.c baseline) with composite-aware perInterface rows.
- LB-TRACK-018 (project-wide diagnose stale errors) → S5 `09c8924` + S5b `28ed311`: `definesActive[]` + `possiblyStale` + `analysisGeneration` on every diagnose/compile_check envelope.
- LB-TRACK-019 (planning-verdict evidence) → S7 `243dd28`: `crossAssemblyConsumerCount: 4, sameAssemblyConsumerCount: 1581, hasSingleImplementer: null` on `AdaptiveBeatGrid`.
- LB-TRACK-020 (remaining trust holes) → composite closure across S5/S5b/S6/S7 + F3a-F3f + F1b + F1d; Unity runtime-discovery hole deferred to a future `UnityReachabilityProvider` v2 wave (explicit limitations[] envelope entries).

Tests 1011 → **1,097 (+86)**, zero skipped. Invariants 101 → **122 (+21)** across 63 → **83 categories (+20)**. Symbols 3,278 → **3,499 (+221)**. Edges 16,973 → **21,121 (+4,148, +24%)** — the bulk of the edge growth is F1d closing the bare-identifier property/event-read gap on Lifeblood self. Types 350 → 363.

### Session 10. Wave G1+G2+G4+R2-3 (2026-04-28)

A single Unity-shaped DAWG audit session surfaced four independent findings; the wave closes all of them as one coherent landing. **Tests 664 → 739 (+75). Invariants 65 → 69 (+4: `INV-EXTRACT-ENUMMEMBER-001`, `INV-RESOLVER-007`, `INV-ANALYZE-FALLBACK-001`, `INV-SEARCH-MATCHKIND-001`).** 0 regressions across the existing suite.

**Findings + fixes:**
1. **R2-3 — silent cross-type substitution on enum-member lookup (DANGEROUS class).** A graph that contained both `field:NS.FieldMask.ShimmerPhase` (enum member) AND `field:NS.BurstVoiceState.ShimmerPhase` (struct field). Asking for the enum member silently returned the struct field — wrong concept, no diagnostic surface, downstream `find_references` walked the wrong target. Two compounding bugs. **Closed both-sided:**
   - **Part A — extractor.** `RoslynSymbolExtractor.ExtractEnum` now walks `EnumDeclarationSyntax.Members` and emits one `Symbol` per member (`Kind = Field`, `Properties[fieldKind] = "enumMember"`, `Properties[constantValue] = ...`). Pre-fix only the enum type entered the graph. Three downstream failure modes followed: exact-ID lookup missed → resolver Rule 4 silent cross-type substitution; References to enum members were dropped by `GraphBuilder`'s dangling-edge filter; dead-code analysis could never observe enum-member usage. Latent dup fix surfaced by enum-member emission: nested types/enums were extracted twice (file-parent + type-parent) and silently deduped — now top-level scan only handles top-level decls. (`INV-EXTRACT-ENUMMEMBER-001`.)
   - **Part B — resolver.** When input parses as a member-kind id (`field:` / `property:` / `method:`), Rule 4 short-name fallback substitutions MUST stay on the same containing-type SHORT name AND the same `Symbol.Kind`. Cross-type / cross-kind hits become `NotFound` with the unfiltered candidates surfaced as `Did-you-mean`. Documented intent of `INV-RESOLVER-005` ("namespace was wrong but the symbol uniquely identified") is preserved. (`INV-RESOLVER-007`.)
2. **G1 — incremental analyze silently widened to full re-analyze on drift.** Pre-fix `RoslynWorkspaceAnalyzer.IncrementalAnalyze` returned `(graph, count)` and silently called `AnalyzeWorkspace` on module-set drift / asmdef drift. Caller learned about cache miss only by noticing wall time. **Closed:** new typed `IncrementalAnalyzeResult { Mode, Graph?, ChangedFileCount, Reason?, Detail? }` + `AnalysisConfig.AllowFullFallback` flag (default `false`). Adapter no longer owns scope-widening policy; caller does. NoPriorAnalysis is always Rejected (no `projectRoot` to widen against without a snapshot). Wire shape: `mode` (DID) + `requestedMode` (ASKED) + `fallbackReason` + `fallbackDetail` + (on rejection) `canRetryFull: true` + `suggestedRetry: { incremental: true, allowFullFallback: true }`. Adapter-agnostic taxonomy (`NoPriorAnalysis` / `ModuleSetChanged` / `ModuleDescriptorChanged`) with adapter-specific descriptor names in `Detail`. (`INV-ANALYZE-FALLBACK-001`.)
3. **G2 — `lifeblood_search` rank source not structurally typed.** Per-bucket scoring signal (name vs qualifiedName vs xmldoc) lived only in human-rendered `MatchSnippets` strings; callers wanting to filter had to parse them. **Closed:** new `MatchKind` enum (`Name` / `QualifiedName` / `XmlDoc` / `Multiple`) on `SearchResult`. Adapter lifts existing first-hit-token tracking into the typed value; no re-scoring, purely additive. Adapter-agnostic — future TS / Python search adapters reuse the same taxonomy. (`INV-SEARCH-MATCHKIND-001`.)
4. **G4 — `authority_report` description framed too narrowly.** The DAWG dogfood case was an ABG partial-class wave, and the description / docs picked up that framing in three places. Reviewers almost skipped using the tool for filter-dispatch widening because the wording made it sound ABG-specific. **Closed:** `ToolRegistry` description names use cases up front (partial-class hosts, dispatchers routing across row paths, facade types, ports with many implementations) + triage heuristics + pairs-with note for `lifeblood_port_health`. `INV-AUTHORITY-001` body, `STATUS.md`, and this file's prose extended to clarify the metric drives any host-with-many-subordinates split decision, not just ABG.

**Wave-end verification ratchets (additional 28 tests):**
- `WaveFunctionalVerificationTests` (4 tests): R2-3 dogfood scenario reproduced as a permanent regression ratchet (graph contains both `FieldMask.ShimmerPhase` AND `BurstVoiceState.ShimmerPhase`, exact-ID lookups return the right one, cross-type query returns NotFound + both candidates as Did-you-mean); all wire-shape states (`full` / `incremental` / `incremental-noop` / `rejected` plus the widened-to-full path) verified through real `GraphSession.Load` JSON; all 4 MatchKind values reachable through real Roslyn extraction.
- `AllToolsSmokeTests` (registry-driven): every MCP tool registered in `ToolRegistry` dispatched through `ToolHandler.Handle` against a real Roslyn workspace, including `lifeblood_execute` running real C# script (`return Graph.Symbols.Count;`) — proves Roslyn integration survived the wave intact. The fixture reads the tool list off the registry at test time so it stays valid as new tools land.

**DAWG end-to-end re-verification:** 60,775 symbols (was 53,882 baseline, +6,893 — explained by enum-member emission), 214,097 edges (+33,283, +18% — explained by enum-member references that the dangling-edge filter was silently dropping pre-fix), 122 cycles (was 117, +5 pre-existing), 47s wall time, 1,145 MB peak. Exit 0, 0 architecture violations.

### Session 9. Post-v0.6.7 polish (2026-04-27)

Six DAWG-dogfood findings shipped on top of v0.6.7 in one session. Every fix carries a regression test; test count 632 → 661 (+29). 0 regressions across the existing suite.

**Findings + fixes:**
1. **`cycles` payload overflow on large workspaces** (`LB-FR-021`). DAWG: 117 SCCs serialize to ~70KB and overflow downstream tool-result limits — the tool succeeded but the wire payload was unconsumable. Fix: same `summarize` + `maxResults` shape as `blast_radius`. Every response carries `count`, `totalSymbolCount`, `largestCycleSize`, `truncated`. `summarize:true` returns a small `preview[]`. Default 25-cycle preview fits comfortably; full 500-cycle response also stays under default budgets.
2. **`invariant_check` parser missed DAWG hot-rule shapes** (`LB-BUG-017` + `LB-BUG-018`). DAWG CLAUDE.md uses three authoring shapes the parser didn't recognize: shape C (`**INV-WORK-001: Title.** body...` — bare bold paragraph, no bullet), shape D (`- **INV-DSP-012** (v1.1.566): body...` — parenthesized version tag), shape E (`- **INV-ANIM-1:** body...` — colon inside the bold). Pre-fix: 0 invariants returned on DAWG. Fix: extend `ClaudeMdInvariantParser` to recognise all five shapes (A/B/C/D/E). Five new ratchet tests per new shape.
3. **`invariant_check` only read `<root>/CLAUDE.md`** (`LB-FR-023`). DAWG moved most invariants out of CLAUDE.md to a `docs/invariants/**.md` tree (the same pattern Lifeblood now follows itself). Fix: `LifebloodInvariantProvider` walks well-known repo conventions dynamically via `IFileSystem` — `<root>/CLAUDE.md`, `<root>/AGENTS.md`, and any `<root>/docs/invariants/**.md` tree. Each source is parsed and aggregated; per-id duplicate detection spans the full set; new `audit.SourcePaths[]` reports every contributing file. The conventions live in the adapter, NOT the port — a repo with a different layout supplies its own provider.
4. **`compile_check filePath` broken on Unity files** (`LB-BUG-019`). Pre-fix `compile_check filePath="Assets/.../AdaptiveBeatGrid.cs"` emitted ~120 spurious CS0246 / CS0103 / CS0538 errors against `UnityEngine`, `MonoBehaviour`, sibling partials, every cross-file type. Root cause: handler read the file content as inline `code`, host added it as a fresh snippet tree to `ResolveCompilation(null)` which picks the first compilation arbitrarily. The file's real owning compilation was never selected. Fix: typed `CompileCheckRequest` overload; file-mode auto-detects the owning compilation by matching the path against every loaded compilation's syntax trees, then `ReplaceSyntaxTree`s the existing tree with the on-disk content. Pre-existing diagnostics in OTHER files in the module are filtered out so only changes the user introduced surface. Post-fix: `success:true, resolvedModule:"Nebulae.BeatGrid.Runtime", existingTreeReplaced:true`.
5. **`context` payload overflow on multi-module workspaces** (`LB-FR-022`). DAWG: full pack ~375KB on 87-module workspace, overflowed tool-result limits. Same fix shape as `cycles`. Per-section caps with sane defaults (25 files / 50 boundaries / 20 hotspots / 50 reading-order / 100 matrix entries), `summarize:true` shortcut, `sections:[...]` allowlist. Every clipped section reported in the response's `truncated` map. Default response now fits comfortably with rich data; `summarize:true` returns the smallest viable shape.
6. **`dead_code` Unity Editor reflection FP class** (`LB-FP-003`). DAWG: `XRaySettingsProvider` falsely flagged dead because the `[SettingsProvider]` attribute (on a contained static method) wasn't in the entrypoint roster, AND the dead-code analyzer didn't propagate liveness from a tagged method to its containing type. Fix: extend the Unity reflection roster (`SettingsProvider`, `SettingsProviderGroup`, `Shortcut`, `OnOpenAsset`, `BurstCompile`, `MonoPInvokeCallback`, full NUnit fixture lifecycle) AND add type-via-child propagation (a type is reachable if any directly-contained member carries an entrypoint attribute). Negative case (plain type with non-entrypoint children) still flags correctly so the propagation doesn't mask real dead code. DAWG type-level dead findings: 6 → 4 (`XRaySettingsProvider` and `MpServiceResets` cleared).

**Repo reorg landed in the same session:** `CLAUDE.md` slimmed 416 → 144 lines. Every formally-numbered `INV-XXX-NNN` rule moved to `docs/invariants/<domain>.md` (8 files + INDEX). Validates the tree-walker shipped in `LB-FR-023` against the Lifeblood repo itself, not just DAWG. `LifebloodClaudeMdSelfTests` aggregates across CLAUDE.md + every tree file, asserts ≥50 invariants, required categories present (DOMAIN, GRAPH, MCP, RESOLVER, CANONICAL, BCL), known stable IDs resolvable, 0 duplicates, 0 parse warnings. Self-analysis post-reorg: 65 typed invariants across 31 categories.
