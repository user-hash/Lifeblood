...
**Current state (v0.7.3-candidate, 2026-05-12, post field-report 2026-05-11 polish wave):** 2,512 symbols, 12,439 edges, 11 modules, 284 types, 0 violations, 0 cycles. Verified on a real 90-module Unity workspace (62,134 symbols, 219,548 edges, 123 SCCs). **26 MCP tools (16 read + 10 write).** **776 tests.** **26 port interfaces.** **80 typed invariants** across 43 categories under `docs/invariants/` tree (queryable via `lifeblood_invariant_check`, zero duplicates, zero parse warnings on Lifeblood self). +4 invariants since v0.7.2: `INV-INCREMENTAL-XREF-001` (cross-module edges survive incremental analyze — closes `LB-BUG-020`), `INV-EDGE-CALLSITE-001` (every expression-derived edge carries first-occurrence authoring provenance), `INV-RESOLVE-MEMBER-001` (`lifeblood_resolve_member` MCP tool — type-scoped member resolution), `INV-BLAST-RADIUS-GROUP-001` (`groupBy=bucket|module|both` response shapes). CallSite provenance verified on 133,523 of 219,548 graph edges (60.8%) on the real Unity workspace — remainder are graph-derived edges with no authoring location, as designed.
...
- `All25ToolsSmokeTests` (24 tests): every one of the 25 MCP tools dispatched through `ToolHandler.Handle` against a real Roslyn workspace, including `lifeblood_execute` running real C# script (`return Graph.Symbols.Count;`) — proves Roslyn integration survived the wave intact.
