...
**Current state (2026-04-27, post P1-P6 + post-v0.6.7 polish):** 2,191 symbols, 10,194 edges, 11 modules, 264 types, 150 files, 0 violations, 0 cycles. Verified on a real 87-module Unity workspace (DAWG, 53,882 symbols, 180,814 edges, 117 SCCs). **25 MCP tools (15 read + 10 write).** All Roslyn capabilities at Proven. **661 tests.** **26 port interfaces.** **65 typed invariants under `docs/invariants/` tree** (8 domain files + INDEX), aggregated by the dynamic tree-walker that also picks up `<root>/CLAUDE.md` + `<root>/AGENTS.md` if they declare any (queryable via `lifeblood_invariant_check`, zero duplicates, zero parse warnings on Lifeblood self; 83 invariants discovered on DAWG across CLAUDE.md hot rules + AGENTS.md project instructions + `docs/invariants/**.md`). Truth envelope on every read-side response (`INV-ENVELOPE-001`). Unity-aware reachability (`INV-UNITY-001`) cuts MonoBehaviour magic-method false positives by 97% on real Unity workspaces, plus the `LB-FP-003` Editor reflection roster + type-via-child propagation closes the `[SettingsProvider]`-on-host-type FP class. Authority report + forwarder classifier (`INV-AUTHORITY-001`, `INV-FORWARDER-001`) classify 18,985 methods on DAWG and identify 3,367 PureForwarder candidates for host-type / dispatcher / partial-class extraction triage (the DAWG dogfood case ran an ABG partial-class wave, but the same metric drives any host-with-many-subordinates split decision — filter family selectors, kernel routers, capability dispatchers, facade types). `cycles` and `context` smart-dynamic shaping (`LB-FR-021`, `LB-FR-022`) keeps responses inside conservative tool-result budgets even on multi-module Unity workspaces. File-mode `compile_check` (`LB-BUG-019`) resolves the owning compilation and swaps the existing tree, so module-owned files compile-check against their real reference set.
