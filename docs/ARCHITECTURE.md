...
- `IModuleDiscovery`. Module and project discovery, returns `ModuleInfo[]`. Each `ModuleInfo` carries `BclOwnership`, `AllowUnsafeCode`, `ImplicitUsings`, `LangVersion`, `Nullable`, `NoWarn`, `DefineConstants`, and `Dependencies` parsed from csproj (csproj-driven compilation facts, `INV-COMPFACT-001..003`).
