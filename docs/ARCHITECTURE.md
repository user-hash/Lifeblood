...
| **Lifeblood.Server.Mcp** | MCP server host. Stdio JSON-RPC. MCP tool surface (18 read + 13 write live in [`STATUS.md`](STATUS.md)). Bidirectional Roslyn. McpDispatcher owns the wire protocol. ToolDefinition.EnvelopeClassification is the registry-side source of truth for the truth envelope. `DotNetDiagnosticsTelemetrySink` is opt-in via `LIFEBLOOD_TELEMETRY` and records tool result, response JSON cost, analyze fallback, truncation, and cache lookup events. | Application, Adapters.CSharp, Connectors |
