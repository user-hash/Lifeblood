...
- **INV-EXTRACT-SYNTHESIZED-CTOR-001. Synthesized `.cctor` and parameterless `.ctor()` surfaces appear as graph Symbols whenever the containing type carries field or property initializers, so initializer-derived edges survive `GraphBuilder`'s source/target filter.** Pre-fix, `RoslynEdgeExtractor.FindContainingMethodOrLocal` attributed every field/property initializer edge to `ContainingType.StaticConstructors.FirstOrDefault()` / `InstanceConstructors.FirstOrDefault()` — for a static class whose only constructor is the runtime-synthesized `.cctor`, that attribution produced edges sourced from `method:NS.T..cctor()`. `GraphBuilder.Build` drops every edge whose source or target is not a Symbol (line ~89, dangling-edge filter, dropping external `System.*` refs from coupling metrics), and the symbol extractor never surfaced synthesized ctors — so every dispatch-table delegate target, every static-initializer enum-member reference, every static-initializer field reference silently disappeared from the graph despite the extractor emitting it correctly. The empirical class: `Registry.HandleA` in a `static readonly Capability[] All = { new(Mode.A, HandleA), ... }` table appeared with `directDependants=0` and surfaced as a `dead_code` finding, despite `find_references` correctly reporting the use. The fix surfaces the synthesizer's view of the type's constructors: `RoslynSymbolExtractor.SurfaceSynthesizedInitializerConstructors` walks every `IFieldSymbol` and `IPropertySymbol` looking for initializer syntax, and when at least one static (or instance) initializer is present, emits a `Method`-kind Symbol carrying the canonical id `method:NS.T..cctor()` / `method:NS.T..ctor()` for the corresponding `IMethodSymbol` whose `DeclaringSyntaxReferences` is empty (Roslyn's marker for synthesized members). The synthesized symbol carries `Properties["synthesized"] = "true"` so downstream `dead_code` excludes it from findings (CLR runtime always invokes `.cctor` on type init / `.ctor()` on first construction — inherently live regardless of static reachability). Symmetric: the instance case only emits when no user-declared parameterless ctor exists (Roslyn synthesizes the default ctor only in that case; a user-declared ctor's body runs field initializers without needing the synthesizer). Pinned by `DispatchTableLivenessRatchetTests.DispatchTableDelegateTargets_AreLiveAcrossGraphWalkSurfaces` — asserts (a) delegate row method has ≥1 incoming graph edge, (b) `dead_code` does NOT flag it, (c) `dead_code` does NOT flag the synthesized `.cctor` itself. Wall ratchet (`BuildDiagnosticParityTests`) still green; the synthesized symbols carry no diagnostic surface and do not produce parity-class findings.

- **INV-EXTRACT-DISPATCH-TABLE-COVERAGE-001. Dispatch-table-shaped static initializers emit graph edges for every method-group, enum-member, and field-reference cell through the same identifier / member-access walkers regular code paths use — no separate static-table-edge synthesizer is needed.** The empirical LB-INBOX-011 part 2 claim that "the graph does not receive a corresponding dependency edge" turned out to be an over-generalization of LB-INBOX-010's target-typed-`new` gap: once `INV-EXTRACT-METHOD-GROUP-CANDIDATE-001` accepts `CandidateSymbols`-bound method groups, every dispatch-table cell (`new Capability(Mode.A, HandleA)`-style) emits the right edge through the same walker that handles `_field = HandleA` outside a table. Method-group cells emit `Calls` from the synthesized `.cctor` to the target (matches the long-standing semantic the explicit `new Lazy<T>(Load)` case has used since v0.6.4); enum-member cells (`Mode.A`) emit `References` from the `.cctor` to the enum-field via `ExtractMemberAccessEdge`; static-field-reference cells (`Constants.Prefix`) emit `References` likewise. All three classes are walked because the AST visitor descends into argument lists regardless of containing-expression shape. Downstream graph-walk tools (`dependants`, `dead_code`, `port_health`, `blast_radius`) all consume both `Calls` and `References` edges through `HasIncomingReference`, so the wire-color distinction (`Calls` vs `References` for delegate storage) is observationally identical at the live-ness layer — committing to a wire-color change would re-ratchet every existing test for ambiguous semantic gain. Pinned by `ExtractEdges_DispatchTableWithMethodGroupAndEnumCells_FullCoverage` (asserts all three cell classes emit their canonical edges from a single mixed-cell-kind dispatch-table fixture).

- **INV-EXTRACT-CROSS-PARTIAL-RESOLUTION-001. Edge extraction across partial-class declarations resolves through the shared Roslyn `Compilation` regardless of which `SyntaxTree` is currently being walked.** When `Service` is declared in `Service.A.cs` (`private void Helper()`) and `Service.B.cs` (`public void Drive() { Helper(); }`), `extractor.Extract(modelB, treeB.GetRoot())` must emit `method:App.Service.Drive() -[Calls]-> method:App.Service.Helper()`. The Roslyn semantic model resolves the call natively because both trees join the same `CSharpCompilation`, but the contract is fragile: any future walker change that scopes resolution to the syntax tree currently being extracted would silently drop every cross-partial edge. Same invariant applies to cross-partial private field reads. Pinned by `ExtractEdges_CrossPartialPrivateMethodCall_EmitsCallsEdgeWithCanonicalId` (file A declares private method, file B partial invokes it) and `ExtractEdges_CrossPartialPrivateFieldReference_EmitsReferencesEdge` (file A declares private field, file B partial reads it). LB-TRACK-20260519-021-adjacent / F1b atom of the 2026-05-19 plan / 2026-05-15 correctness masterplan Stage 1.

- **INV-EXTRACT-EXTENSION-REDUCED-001. Extension-method canonical ids walk through `ReducedFrom` before `OriginalDefinition`, so instance-style invocations (`x.Foo()`) and static-style invocations (`Helper.Foo(x)`) emit graph edges with byte-identical ids.** Pre-fix, instance-style extension invocation bound to the REDUCED `IMethodSymbol` whose parameter list drops the explicit `this` receiver; `RoslynEdgeExtractor.GetMethodId` and `RoslynCompilationHost.BuildSymbolId` then built the canonical id off that reduced shape (`method:NS.Ext.Foo()` — no params) while the declaration path emitted the unreduced form (`method:NS.Ext.Foo(string)`). Every instance-style call-site landed on a non-matching id, so the declared extension method showed `directDependants:0` and could surface as a `dead_code` finding even when the workspace called it heavily — a defect class strictly equivalent to LB-INBOX-010's generic-method drift but specialized to extension methods. Both producer-side (`GetMethodId`) and consumer-side (`BuildSymbolId`) now route through `if (method.ReducedFrom != null) method = method.ReducedFrom;` before `OriginalDefinition` canonicalization, so the reduced-then-unreduced pipeline is symmetric across the two id-building paths. Fix order matters: `ReducedFrom` first (recover the explicit `this` receiver in `T` / closed form), then `OriginalDefinition` (recover the open-generic form for generic extension methods). Pinned by `ExtractEdges_ExtensionMethodInstanceCall_AttributesToUnreducedId` (instance-style round-trip), `ExtractEdges_ExtensionMethodStaticCall_AttributesToUnreducedId` (static-style parity), `ExtractEdges_GenericExtensionMethodInstanceCall_AttributesToOpenGenericUnreducedId` (composition with `OriginalDefinition` for `Pick<T>(this T[] items, int cap)`), and `ExtractEdges_ChainedExtensionMethodCalls_BothCallsAttributeToUnreducedId` (the two-extension instance-chain shape). LB-TRACK-20260519-021.
