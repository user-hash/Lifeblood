...
- **INV-EDGE-CALLSITE-001. Every expression-derived edge carries authoring provenance.** `RoslynEdgeExtractor.BuildCallSite` emits a `Lifeblood.Domain.Graph.CallSite` (`FilePath` / `Line` / `Column` / `EndLine` / `EndColumn` / `ContainingSymbolId`) for every edge sourced from a Roslyn `SyntaxNode` — invocation operations, member-access expressions, field references, property accesses, etc. `Edge.CallSite` is null for graph-derived edges with no single authoring location (module→module `DependsOn`, type→type `Inherits` without a surfaced clause node, type-level edges synthesised at graph-construction time). The JSON graph importer / exporter round-trip the field so a cached graph preserves provenance across save / load. Downstream tools (`lifeblood_dependencies` / `lifeblood_dependants` wire surface, future tools) lift this directly without re-walking files — one call answers "where in source does X depend on Y?". Pinned by `EdgeCallSiteTests.Extract_CallEdge_AttachesCallSiteWithFileLineColumn`, `Extract_FieldReference_AttachesCallSite`, `JsonGraph_RoundTripsCallSite`. Closes the field-report 2026-05-11 P1 CallSite ask.
