...
**Step 4. Open Unity.** The Editor compiles the bridge stubs. Coplay's MCP plugin auto-discovers them via the `[McpForUnityTool]` attribute. The Unity bridge surfaces a curated subset — 18 of the 33 tools (the in-Editor-relevant read + write ones, with `lifeblood_analyze_project` wrapping analyze) — in Coplay's tool list alongside the built-in Unity tools. The remaining tools (`lifeblood_capabilities`, `lifeblood_search`, `lifeblood_dead_code`, `lifeblood_port_health`, `lifeblood_cycles`, `lifeblood_enum_coverage`, `lifeblood_static_tables`, `lifeblood_assignment_coverage`, and others) are reachable by connecting an MCP client directly to the standalone `lifeblood-mcp` server (see the client sections above).
