...
### Dead-Code Analysis Limitation (v0.6.3)

- **INV-DEADCODE-001. `lifeblood_dead_code` is advisory and ships with known false-positive classes. Every response carries a `status: "experimental"` + `warning` field so agents cannot consume findings without seeing the caveat.** The analyzer walks `graph.GetIncomingEdgeIndexes(sym.Id)` and flags symbols whose incoming-edge set is empty after filtering `EdgeKind.Contains`. The logic is correct given a complete graph. But during the v0.6.3 release checkup the dogfood pass found three classes of false positive in real multi-module workspaces:

  1. **Method-group references.** A private method passed as a delegate. `new Lazy<MetadataReference[]>(Load)`, event handler registration, LINQ `Where(predicate)`. never produces a direct `InvocationExpressionSyntax` at the call site, so `RoslynEdgeExtractor.ExtractCallEdge` does not emit a `Calls` edge into the referenced method. The method appears orphaned even though it is clearly used. `Lifeblood.Adapters.CSharp.Internal.BclReferenceLoader.Load` is the canonical example: only referenced via `new Lazy<>(Load)` in the same type's static field initializer.

  2. **Multi-module canonical-id drift.** A direct invocation. `CreateCompilation(module, projectRoot, config, depRefs, skippedCollector)` on line 96 of `ModuleCompilationBuilder.cs`. correctly resolves via Roslyn's `model.GetSymbolInfo(invocation)` and passes every guard in `ExtractCallEdge`, but the resulting `Calls` edge does NOT appear in the graph for reasons that do not reproduce in isolated single-file or two-module synthetic tests. The canonical ID stored by `RoslynSymbolExtractor` for the definition (`method:...CreateCompilation(...,List<Lifeblood.Domain.Results.SkippedFile>?)`) matches the resolver output but `lifeblood_find_references`, `lifeblood_dependants`, and `lifeblood_dead_code` all report zero incoming references. The bug likely lives in either (a) nullability or generic-type display format drift when the parameter type comes from a metadata PE reference in a multi-module build, or (b) something in the full-workspace compilation pipeline that isolated test compilations do not exercise. Root cause investigation deferred to v0.6.4.

  3. **Same-class private field reads.** Private fields accessed only from sibling methods on the same type (`_fs`, `_nuget`, `_refCache` in `ModuleCompilationBuilder`) are flagged because the extractor does not emit field-read edges at the method-to-field granularity.

  The `dead_code` tool ships in v0.6.3 with the `[EXPERIMENTAL. ADVISORY ONLY]` prefix on its description, every response carries a `status: "experimental"` marker and a `warning` text listing the three false-positive classes, and the CHANGELOG's v0.6.3 entry flags the limitation prominently. The tool still surfaces real dead code for the majority of non-public non-test symbols outside the three gap classes; consumers are expected to verify every finding with `lifeblood_find_references` (which has the same gap class for category 2 but not for 1 and 3) and direct code inspection. The architectural position is: ship the tool with honest caveats rather than ship no tool at all; the follow-up root-cause investigation has dedicated v0.6.4 scope. Pinned by the handler test `Handle_DeadCode_Response_IncludesExperimentalWarning` (v0.6.3, added alongside this invariant) and by the two diagnostic unit tests `ExtractEdges_MethodCall_NullableGenericParameter_SameClass_ProducesCallsEdge` and `ExtractEdges_MethodCall_ComplexSignature_MatchesRealProcessInOrder` in `RoslynExtractorTests` (both currently passing in isolation; they document the pattern that needs to work and will catch the narrow synthetic case if a regression breaks it).
