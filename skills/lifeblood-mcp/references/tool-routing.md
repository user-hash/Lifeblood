...
| Enforce consumer-owned guards, external API costs, value/shape, lifecycle, smoothing, or discontinuity policy? | `lifeblood_contract_audit` | One versioned manifest, one bounded operation-fact scan, explicit per-contract coverage, summary-first findings; no extra graph or semantic base. |
