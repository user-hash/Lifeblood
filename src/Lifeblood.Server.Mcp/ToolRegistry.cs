...
public static class ToolRegistry
{
  ...
  Description = "Evaluate a schema-versioned consumer contract manifest over one selector-bounded neutral operation-fact stream. Supply exactly one of inline `manifest` or workspace-contained `manifestPath`. Typed rule families cover operation-argument guards, consumer-annotated external API costs, value domains with exact conversions/non-finite/literal/cadence policy, and operation shapes with exact multi-input, result-type, lexical-control, constant-range, and request-local uniqueness policy; exact reasoned suppressions live in the same manifest. One scan serves every selected rule, adds no graph edges, and retains no additional semantic base. Summary-first by default: at most 25 findings with evidence omitted; pass `summarize:false` plus explicit caps for bounded detail. Non-primary committed profiles compile only the requested module scope ephemerally after input-identity verification. INV-CONTRACT-AUDIT-001.",
  },
  new()
  {
