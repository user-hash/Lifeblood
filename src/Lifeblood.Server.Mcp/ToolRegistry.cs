...
public static class ToolRegistry
{
  ...
  maxMatrixEntries = new { type = "integer", description = "Cap on dependencyMatrix entries (module-to-module edges). Default 100. The full matrix on an 80+-module workspace is ~2600 entries." },
  },
  },
  },
  new()
  ...
  summarize = new { type = "boolean", description = "When true, omit the full `findings[]` array and return only counts + `kindBreakdown` + a small `preview[]` (size capped by maxResults). Defaults to false." },
  maxResults = new { type = "integer", description = "Maximum number of findings embedded in the response. When the finding set is larger, the array is clipped and `truncated:true` is set. Default: 500 in normal mode, 25 in summarize mode." },
  },
  },
  },
  new()
