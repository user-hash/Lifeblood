...
public static class ToolRegistry
{
  ...
  includeReflectionHeuristic = new { type = "boolean", description = "Default false. When true, after the BFS completes, scan each test method's containing file for the target's FQN as a source-text substring (with namespace-context or uniqueness-gated short-name fallback). Hits surface as `kind: ReflectionHeuristic` rows. Symbol targets only — ignored when `target` is a file path." },
  },
  },
  },
  new()
