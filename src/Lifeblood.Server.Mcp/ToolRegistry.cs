...
public static class ToolRegistry
{
  ...
  allowFullFallback = new { type = "boolean", description = "Pairs with `incremental:true`. When true, the adapter silently widens to a full re-analyze on detected drift and reports `mode:'full-fallback'` plus the fallback reason. When false (default), the adapter REJECTS with `mode:'rejected'` + reason and does no work — caller decides next step. Eternal-repo posture: scope policy is the caller's choice, not the adapter's. INV-ANALYZE-FALLBACK-001." },
  },
  },
  },
  new()
