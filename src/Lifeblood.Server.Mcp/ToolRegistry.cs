...
{
  // Typed behavior presets. Every tool chooses exactly one immutable contract;
  // session prerequisites, externally meaningful effects, and gate access are
  // never inferred from names or authored in a second dispatch table.
  private static readonly ToolBehavior CapabilityObservation = new(
    ToolSessionRequirement.None,
    ToolEffect.Observe,
    ToolSessionAccess.SharedRead);

  private static readonly ToolBehavior WorkspaceRefresh = new(
    ToolSessionRequirement.None,
  ...
  private static readonly ToolBehavior WorkspaceRootObservation = new(
  ...
  private static readonly ToolBehavior CompilationRefresh = new(
    ToolSessionRequirement.RetainedCompilation,
    ToolEffect.RefreshWorkspace,
    ToolSessionAccess.Exclusive);

  private static readonly ToolBehavior CodeExecution = new(
    ToolSessionRequirement.RetainedCompilation,
    ToolEffect.ExecuteCode,
    ToolSessionAccess.SharedRead);

  private static readonly ToolBehavior ChangePreview = new(
    ToolSessionRequirement.RetainedCompilation,
    ToolEffect.PreviewChanges,
    ToolSessionAccess.SharedRead);
  ...
  // Every tool registration sets one of these on its
  ...
  private static readonly EnvelopeClassification HeuristicAdvisoryDeadCode = new()
  {
    ...
    Limitations = new[]
    {
      "Dead-code is reachability-only — runtime entry points (Program.Main), unresolved Unity serialized callbacks/asset references, and reflection-based dispatch not modeled by the active reachability provider may surface as false positives.",
    },
  };
  ...
  // Unity-runtime dispatch invisible to static analysis (unresolved UnityEvent
  // YAML bindings, serialized-reference fields on prefabs/scenes/SOs,
  ...
  /// response. Availability decoration is evaluated from each definition's
  /// typed session requirement. The returned array is a pure
  /// <see cref="McpToolInfo"/> wire DTO — behavior metadata does not leak onto
  /// the MCP-standard tool shape.
  ///
  /// <para>
  /// INV-TOOLREG-001: state, effect, and session access come from the required
  /// <see cref="ToolDefinition.Behavior"/> contract, never tool-name prefixes.
  /// </para>
  /// </summary>
  public static McpToolInfo[] GetTools()
  ...
  /// compilation state. It assumes an analyzed workspace, matching the old
  /// method's contract.
  /// </summary>
  public static McpToolInfo[] GetTools(bool hasCompilationState)
    => GetTools(new ToolSessionState(
      HasAnalyzedWorkspace: true,
      HasWorkspaceRoot: true,
  ...
  if (!sessionState.Satisfies(def.Behavior.SessionRequirement))
  {
  description = UnavailablePrefix(def.Behavior.SessionRequirement) + description;
  }
  ...
  private static string UnavailablePrefix(ToolSessionRequirement requirement)
    => requirement switch
    {
      ToolSessionRequirement.AnalyzedWorkspace =>
  ...
  /// <summary>
  /// Returns the internal tool definitions with their full behavior metadata.
  /// The array is copied; the immutable definitions are registry-owned.
  /// </summary>
  public static ToolDefinition[] GetDefinitions() => Definitions.ToArray();

  /// <summary>
  /// Finds a registered definition without manufacturing another behavior
  /// table at the dispatch boundary.
  /// </summary>
  public static ToolDefinition? FindDefinition(string name)
    => DefinitionsByName.GetValueOrDefault(name);

  private static readonly ToolDefinition[] Definitions = BuildDefinitions();
  ...
  private static ToolDefinition[] BuildDefinitions() => new ToolDefinition[]
  {
  new()
  {
  ...
  Description = "Report the live Lifeblood MCP server capabilities: server version and version source, optional git commit/dirty state when running from a repo checkout, MCP tool count with legacy read/write split plus typed behavior contracts (session requirement, effect, access), feature flags, schema snapshot path, STATUS.md anchor path, and current session state. Use this at session start to detect local-tool/documentation drift before relying on stale prose.",
  },
  new()
  {
  ...
  Description = "Analyze a C# project or JSON graph file. Returns symbol/edge/module counts and violations. Loads the graph into memory for subsequent query tools. Wire shape: `mode` reports what the adapter DID (`full` / `incremental` / `incremental-noop` / `rejected`), `requestedMode` separately reports what the caller ASKED (`full` / `incremental`). When the cheap path could not be honored cleanly the response carries `fallbackReason` (`noPriorAnalysis` / `moduleSetChanged` / `moduleDescriptorChanged` / `analysisScopeChanged`) + a human-readable `fallbackDetail`. By default `incremental:true` REJECTS when drift is detected (caller-owned scope policy, INV-ANALYZE-FALLBACK-001) — the rejection is a normal structured result, not a tool error: it carries `canRetryFull:true` and a `suggestedRetry` block (`{incremental:true, allowFullFallback:true}`) so the agent can retry without out-of-band knowledge. Pass `allowFullFallback:true` up front to opt into silent widening; you'll receive `mode:'full'` + `requestedMode:'incremental'` + the populated `fallbackReason` so the cache miss stays visible. `publication.action` says whether this response published a new generation or reused the current exact identity (`reusedCurrent`), preventing duplicate-looking retry evidence. `profileApplicability` appears on multi-profile / excluding profile runs and reports adapter-owned module applicability counts by default; pass `profileApplicabilityMode:'detail'` for the per-module ledger with module name, project file, raw UnityProjectType, included/excluded profiles, and exclusion reasons. `excludePaths` accepts project-relative POSIX globs that remove files before Roslyn compilation; changing that set during incremental analyze is an analysis-scope drift and reports `analysisScopeChanged` unless full fallback is allowed. Responses report both `changedSourceFiles` (number of .cs files that re-extracted this round) and `touchedGraphFiles` (how many graph entries were rebuilt) — currently the same value, surface kept stable for future divergence. `acceptedChanges.interpretation` spells out whether that work was source content change, mtime-only touch, descriptor recompile, deletion, or full fallback reanalysis; full fallback says `work:'fullFallbackReanalysis'` and `contentChangeStatus:'none'` when source content did not change. Unity note: Lifeblood discovers source files through the generated project descriptors (`.csproj` / asmdef membership), not a raw disk sweep. On Unity package workspaces the response carries `packageSourceVisibility`: package roots from `Packages/manifest.json`, `Packages/packages-lock.json`, and embedded package descriptors; package asmdefs; source counts; and included/excluded/unbound status counts derived from descriptor membership plus `excludePaths`. Default `packageSourceVisibilityMode:'summary'` returns aggregate counts plus only actionable excluded/unbound package rows and omits per-file inventories; pass `packageSourceVisibilityMode:'detail'` for every package row with bounded file previews (`INV-UNITY-PACKAGE-SOURCE-001`). A `.cs` file written to disk that Unity has not yet imported (no `.meta` sibling, not yet in any generated `.csproj`) is in no module, so analyze (full or incremental) will not include it until Unity regenerates the project files — the next analyze then picks it up. `compile_check(filePath)` on such a file returns `fileResolution: NotInAnyCompilation` with a `staleDescriptorHint` naming the regenerate-then-reanalyze step. If Unity later assigns the file a different GUID, the next full analyze refreshes all symbol IDs. INV-UNITY-NEWFILE-DISCOVERY-HONESTY-001 / INV-ANALYZE-EXCLUDEPATHS-001.",
  },
  new()
  {
  ...
  Description = "Get file-level impact: which files depend on this file and which files this file depends on. Derived from symbol-level edges. Answers 'if I change this file, what other files are affected?' Every response carries `dependsOnCount` + `dependedOnByCount` (full graph magnitudes — stay byte-stable even when arrays clip) alongside the per-direction arrays. Pass `maxResults` to cap each direction's array (default 500; clipped arrays fire `dependsOnTruncated` / `dependedOnByTruncated` flags plus a composite `truncated` bool for one-field checks). Pass `summarize:true` (INV-FILE-IMPACT-SUMMARIZE-001) for the smallest viable wire shape — forces `maxResults=25` regardless of caller-passed value; mirrors the summarize shortcut already shipped on `dead_code`, `cycles`, `blast_radius`, `test_impact`. Pass `includeUnsupportedRelationships:true` to add a capped advisory `unsupportedRelationships` receipt: semantic impact counts stay graph-proven, while source-file IO literal hits such as `File.ReadAllText(\"Target.cs\")`, graph-resolved reflection type strings such as `Type.GetType(\"Namespace.Target\")`, and documented blind-spot families (Resources.Load paths, Unity serialized references) are reported separately. Useful on god-type primary partial files (high fan-out / high fan-in) where the full enumeration overflows downstream tool-result budgets.",
  },
  new()
  {
  Name = "lifeblood_asmdef_check",
  Behavior = GraphObservation,
  EnvelopeClassification = DerivedProven,
  Description = "Unity asmdef compile-direction check. For every cross-module source edge whose source module is marked `referenceClosure=DirectOnly`, verifies that the source module declares a direct module dependency on the target module. Reports one violation per source-target module pair with the first offending edge, callSite, profile set, declared dependency list, and offending-edge count. SDK-style / transitive modules are skipped because transitive ProjectReference closure is valid there. Pass `summarize:true` for a compact capped result. INV-ASMDEF-CHECK-001.",
  },

  // ── Retained-compilation tools ──
  //
  // Visual grouping only. The authoritative classification is the
  // ToolDefinition.Behavior on each record, not this comment.

  new()
  {
  Name = "lifeblood_execute",
  Behavior = CodeExecution,
  ...
  // Graph observation: consults SemanticGraph.GraphIndexes.FindByShortName (the
  ...
  // tool sat under the retained-compilation divider but none of the
  ...
  Description = "[EXPERIMENTAL — ADVISORY ONLY] Scan the loaded graph for symbols with no incoming semantic references — dead code CANDIDATES. Defaults: excludes public-visibility symbols (assumed reachable from outside) and test files. Returns canonical ids, kinds, file:line locations, and a short reason per hit. Every finding additionally carries `directDependants` (incoming non-Contains edge count; 0 for classic findings but forward-compatible signal for future relaxed criteria), `bucket` (one of `Production` / `Test` / `Editor` / `Generated` / `Vendored` path buckets, or the dead_code-only `Scaffolding` downrank for intentional conditional/const-string anchor types), and `declarationOnly` (true for abstract/interface members — deleting one is a public-contract change). Every response carries `count`, `kindBreakdown` (per-symbol-kind histogram), `bucketBreakdown` (per-bucket histogram so a caller can fold the Editor/Generated/Vendored/Scaffolding tail in one pass), and `truncated`. Use `includeKinds` to narrow (e.g. ['Method']). Use `summarize:true` for a compact response with `preview[]` instead of the full `findings[]` — useful on large workspaces (53k+ symbols → 286KB+ payload) which would otherwise overflow downstream tool-result limits. `maxResults` caps the embedded array regardless of mode (default 500, or 25 in summarize). **Closed false-positive classes (INV-DEADCODE-001):** v0.6.4 closed five extractor classes (interface dispatch, member-access granularity, null-conditional property, lambda context, method-group references) plus the implicit-global-usings compilation gap. v0.6.5 closed three more (constructor `Calls` edge, field-initializer containing method, property-accessor body context). v0.6.7 closed the MonoBehaviour magic-method class on Unity workspaces via the `IUnityReachabilityProvider` port (`INV-UNITY-001`); the current extractor records resolved `baseTypeChain` facts so metadata-only intermediate bases such as Unity UI Graphic/UIBehaviour still reach MonoBehaviour. v0.7.0 (`LB-FP-003`) closed the Editor reflection-attribute class (`[SettingsProvider]`, `[Shortcut]`, `[OnOpenAsset]`, `[BurstCompile]`, `[MonoPInvokeCallback]`, full NUnit fixture lifecycle) plus type-via-child propagation. Current dead_code also downranks intentional reference-free scaffolding (`INV-DEADCODE-SCAFFOLDING-001`) and marks resolved UnityEvent persistent-call targets reachable from prefab/scene/asset YAML (`INV-UNITYEVENT-REACHABILITY-001`). **Remaining advisory limitations:** runtime entry points (Program.Main); reflection-based dispatch invisible to static analysis (Type.GetType + MethodInfo.Invoke, Unity SendMessage); Unity serialized callbacks whose target type/script cannot be resolved from YAML; UI Toolkit VisualElement subclasses with magic-named methods on non-MonoBehaviour bases; private fields read via same-class access when the enclosing type has no external references. Treat every finding as a candidate to verify with `lifeblood_find_references` before acting. Triage fields per INV-DEADCODE-TRIAGE-001.",
  },
  new()
  {
  ...
  Behavior = WorkspaceRootObservation,
  EnvelopeClassification = SemanticProven,
  Description = "Query the architectural invariants declared in the loaded project's invariant tree. The provider walks <root>/CLAUDE.md, <root>/AGENTS.md, and any <root>/docs/invariants/**.md, aggregating across every source. Three modes: (1) pass 'id' to fetch one invariant's full body, title, category, and source line; (2) pass mode='audit' (default) for totals, per-category counts, duplicate-id collisions, parse warnings, contributing sources, and coverage/coverageWarnings that distinguish prose-only or under-recognized trees from parser errors; audit summarize=true keeps those actionable facts while projecting nonzero source counts exactly once through the citation receipt instead of duplicating the zero-heavy ledger; (3) pass mode='list' for an id/title index across every declared invariant. Requires a prior lifeblood_analyze to establish the project root.",
  },
  new()
  {
  ...
  new()
  {
  Name = "lifeblood_authority_coverage",
  Behavior = GraphObservation,
  EnvelopeClassification = DerivedProven,
  Description = "Negative-dependency matrix for source-of-truth coverage. Inputs are `subjects[]` (methods/types/files), `requiredAuthority[]` (symbols/types/namespaces/files), optional `allowedAlternatives[]`, `maxDepth`, and subject bucket filters. Types/files expand to contained methods before traversal; authorities expand to themselves plus contained symbols, so reaching a method/property on an authority type counts as reaching that authority. For each subject row, walks outgoing non-Contains edges and reports reached authorities, missing authorities, shortest path previews, and the first allowed alternative actually used. Graph-only and evidence-first: catches 'wired to the wrong or incomplete source of truth' without declaring architectural guilt. INV-AUTHORITY-COVERAGE-001.",
  },
  new()
  {
  ...
  Description = "Evaluate a schema-versioned consumer contract manifest over one selector-bounded neutral operation-fact stream. Supply exactly one of inline `manifest` or workspace-contained `manifestPath`. Typed rule families cover operation-argument guards, consumer-annotated external API costs, call-route shared-state classification, route-fact required/parity/exclusive-owner policy, value domains with exact conversions/non-finite/literal/cadence policy, and operation shapes with exact multi-input, result-type, lexical-control, branch-arm, constant-range/forbidden-constant, and request-local uniqueness policy; exact reasoned suppressions live in the same manifest. Route-fact signatures compare only consumer-selected dimensions across manifest-declared routes; they do not infer behavioral equivalence or execution lanes. Rule breakdowns expose per-contract evaluated/finding-free/finding/suppressed counts so an empty match set cannot look like a pass. `sourceTextPolicies[]` and `invariantEvidence[]` reuse the same publication and manifest: caller-owned retired terms stay Advisory, while invariant selectors return explicit selector counts, named declaration/source/test/reachable-test/operation/caller-declared categories, concrete gaps, and no global score. Evidence-only manifests report operation execution as `NotRequested`. At most one operation scan and one request-local source-evidence pass serve all selected rules; neither adds graph edges or a retained semantic base. Summary-first by default: at most 25 findings with evidence omitted; pass `summarize:false` plus explicit caps for bounded detail. Non-primary committed profiles compile only the requested module scope ephemerally after input-identity verification. INV-CONTRACT-AUDIT-001.",
  },
  new()
  {
  ...
  Description = "Dead-WIRE audit — members that compile green and are REFERENCED but are structurally unplugged at runtime. The complement of `lifeblood_dead_code` (which finds UN-referenced symbols): this catches the opposite failure, the recurring extraction-severed-wiring bug class. One operation-tree pass over every loaded compilation classifies each field/property reference as read or write (assignment target, ++/--, ref/out arg, or declaration initializer = write; else read) and accumulates per-member counts. Passes: `FieldReadWithoutWrite` = private/internal mutable field READ at >=1 site with zero write sites (no assignment anywhere, no initializer) — 'forgot to wire it'; `DelegateSlotNeverAssigned` = delegate-typed (Func/Action/custom-delegate) mutable field or property with zero assignment sites — a binding/callback slot that nothing ever fills; `EventSubscribedNeverRaised` = event with subscribers (+=) but zero raise sites — handlers attached, nothing fires it; `EventRaisedNeverSubscribed` = event raised but with zero subscribers — fired into the void; `DegenerateConstantCallSites` = private/internal method whose every call site passes only compile-time-degenerate args (constant / default / null) — a vestigial parameter or placeholder wire (field-based sentinels like Vector2.zero are NOT degenerate and not detected). Each finding carries memberId, memberKind (Field/Property/Event/Method), memberType, declaringTypeId, file:line, readCount, writeCount (generic reference tallies — for an event raise/subscribe sites, for a method total call sites), and a reason. Response carries `kindBreakdown` + `findingCount` + `truncated`. ADVISORY (Heuristic envelope): a member wired only through reflection, Unity serialized YAML (UnityEvent / [SerializeField]), or runtime-procedural assignment looks unplugged here — verify before acting. Optional `typeId` / `moduleScope` filter the FINDINGS (counting always scans all compilations); `includeFieldReadWithoutWrite` / `includeDelegateSlots` / `includeEvents` / `includeDegenerateConstantCallSites` toggle the four pass families; `maxFindings` (default 200) clamps; `summarize:true` forces the compact 25-finding triage cap (`kindBreakdown` still counts all). INV-WIRE-AUDIT-001.",
  },
  new()
  {
  ...
  new()
  {
  Name = "lifeblood_struct_layout",
  Behavior = CompilationObservation,
  EnvelopeClassification = SemanticProven,
  Description = "Compute metadata layout for a struct: layout kind, effective pack, declared size, total size, alignment, pointer size, unmanaged/blittable flags, and one row per instance field with offset / size / alignment / fixed-buffer length. Exact for known blittable Sequential / Explicit structs (primitive numeric fields, enums, nested structs, fixed buffers, pointers); downgraded to Advisory with `limitations[]` for LayoutKind.Auto, reference-bearing fields, non-blittable primitives such as bool/char, and unknown metadata shapes. Uses the same resolve-from-source gate as `lifeblood_member_count`; `typeId` accepts canonical (`type:NS.T`), qualified, or short name. INV-STRUCT-LAYOUT-001.",
  },
  new()
  {
