...
public static class ToolRegistry
{
  ...
  Description = "Evaluate a schema-versioned consumer contract manifest over one target-filtered neutral operation-fact stream. Supply exactly one of inline `manifest` or workspace-contained `manifestPath`. Typed rule families cover operation-argument guards, consumer-annotated external API costs, and value domains with exact conversions, non-finite policy, literal provenance, consumer-toleranced near-equal groups, and lexical cadence-boundary shapes; exact reasoned suppressions live in the same manifest. One scan serves every selected rule, adds no graph edges, and retains no additional semantic base. Summary-first by default: at most 25 findings with evidence omitted; pass `summarize:false` plus explicit caps for bounded detail. Non-primary committed profiles compile only the requested module scope ephemerally after input-identity verification. INV-CONTRACT-AUDIT-001.",
  },
  new()
  {
