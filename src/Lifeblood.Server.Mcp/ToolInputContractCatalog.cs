...
public static class ToolInputContractCatalog
{
    ...
    private static IEnumerable<ToolInputContract> Create()
    {
        ...
        yield return Contract(@"lifeblood_contract_audit",
            Arg(@"manifest", ToolArgumentType.Object, required: false, arrayItemType: null, description: @"Inline schema-versioned contract manifest. Families include bounded callRoutes, operationGuards, externalApiCosts, valueDomains, and exact operationShapes for multi-input/result/control/branch-arm/constant/uniqueness policy. External-cost contracts may select direct/transitive occurrences through callRouteIds and use explicit matchAnyTarget for targetless operation kinds. Supply exactly one of manifest or manifestPath.", enumValues: Array.Empty<string>()),
