...
public static class ToolInputContractCatalog
{
    ...
    private static IEnumerable<ToolInputContract> Create()
    {
        ...
        yield return Contract(@"lifeblood_performance_evidence",
            Arg(@"action", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Runtime-evidence operation: import (default) validates and summarizes one capture; correlate joins one capture to the leased graph; compare validates two capture identities before returning normalized deltas.", enumValues: new[] { @"import", @"correlate", @"compare" }),
            Arg(@"workspaceRoot", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Absolute workspace directory used to contain runtime-evidence paths. Required for import/compare when no analyzed workspace is loaded; otherwise defaults to the current workspace and must match it when supplied. Correlate always uses and, when supplied, must match the current source-evidence publication root.", enumValues: Array.Empty<string>()),
            Arg(@"sourcePath", ToolArgumentType.String, required: true, arrayItemType: null, description: @"Generic JSON/CSV or structural Unity ProfilerRecorder receipt inside workspaceRoot. Relative paths resolve from that root.", enumValues: Array.Empty<string>()),
            Arg(@"candidatePath", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Second workspace-contained capture required by action:compare.", enumValues: Array.Empty<string>()),
            Arg(@"format", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Optional source format hint. Auto-detected from content/path when omitted.", enumValues: new[] { @"json", @"csv" }),
            Arg(@"candidateFormat", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Optional candidate format hint for action:compare.", enumValues: new[] { @"json", @"csv" }),
            Arg(@"comparisonMode", ToolArgumentType.String, required: false, arrayItemType: null, description: @"crossDevice (default) accepts reported device hardware differences while still gating scenario/build/platform/API/workload identity; strict requires identical device identity.", enumValues: new[] { @"crossDevice", @"strict" }),
            Arg(@"allowedDifferences", ToolArgumentType.Array, required: false, arrayItemType: ToolArgumentType.String, description: @"Explicit environmental fields accepted as different (width, height, refreshRateHz, or device fields in strict mode). Core scenario/build/workload gates cannot be bypassed.", enumValues: Array.Empty<string>()),
            Arg(@"markerAliases", ToolArgumentType.Array, required: false, arrayItemType: ToolArgumentType.Object, description: @"Caller-owned marker mappings shaped as { marker, symbolId }. Exact aliases outrank capture hints, symbol names, and string-literal ownership; conflicting aliases remain Ambiguous.", enumValues: Array.Empty<string>()),
            Arg(@"maxMeasurements", ToolArgumentType.Integer, required: false, arrayItemType: null, description: @"Maximum imported measurement rows per capture. Default 10000; hard cap 100000. Complete observed count and truncation remain visible.", enumValues: Array.Empty<string>()),
            Arg(@"maxMarkers", ToolArgumentType.Integer, required: false, arrayItemType: null, description: @"Maximum correlation rows. Default 25 in summary mode or 200 in detail; hard cap 1000.", enumValues: Array.Empty<string>()),
            Arg(@"maxTestsPerSymbol", ToolArgumentType.Integer, required: false, arrayItemType: null, description: @"Maximum affected test classes projected per correlated symbol. Default 3 in summary mode or 10 in detail; hard cap 50.", enumValues: Array.Empty<string>()),
            Arg(@"maxDeltas", ToolArgumentType.Integer, required: false, arrayItemType: null, description: @"Maximum matched numeric delta rows. Default 50 in summary mode or 500 in detail; hard cap 5000.", enumValues: Array.Empty<string>()),
            Arg(@"summarize", ToolArgumentType.Boolean, required: false, arrayItemType: null, description: @"Summary-first default true. Keeps identity/census complete while bounding marker, test, measurement-preview, and delta arrays.", enumValues: Array.Empty<string>())
        );
