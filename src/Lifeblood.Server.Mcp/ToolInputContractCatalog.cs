...
public static class ToolInputContractCatalog
{
    ...
    private static IEnumerable<ToolInputContract> Create()
    {
        ...
        yield return Contract(@"lifeblood_analyze",
            ...
            Arg(@"packageSourceVisibilityMode", ToolArgumentType.String, required: false, arrayItemType: null, description: @"Unity package visibility projection. `summary` (default) returns package/source counts without per-file inventories; `detail` returns bounded per-package file previews for investigations that need exact source rows.", enumValues: new[] { @"summary", @"detail" }),
