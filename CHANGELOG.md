...
- **Status doc + DocsTests anchors refreshed.** `<!-- testCount: 1097 -->` (was 1011), invariant count 122 across 83 categories (was 101/63), self-analyze 3366/17048/363/0/0 (symbols/edges/types/violations/cycles). Tools / ports unchanged at 29 / 27.

- **Tracker sweep (LB-TRACK 012-024 → Shipped).** Every open atom from the 2026-05-19 plan now carries a closing-commit reference plus DAWG live-MCP re-verification body. LB-TRACK-023 remains Partially Shipped per entry body (F1c closed the empirical `.ctor`/`.cctor` parser bug; deferred sub-atom is an SSoT consolidation parity-matrix ratchet — no known user-facing defect, architectural hygiene only).
